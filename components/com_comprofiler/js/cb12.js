...
function cbParamChange() {
	...
	if ( typeof( cbjQuery ) == 'undefined' ) {
		...
		for ( i = 0; i < fieldsToHide.length; i++ ) {
			...
			if ( field ) {
				if ( ( field.type == 'radio' ) || ( field.type == 'checkbox' ) ) {
					field.parent.style.display = 'none !important';
					field.checked = false;
				} else {
		...
		for ( i = 0; i < fieldsToShow.length; i++ ) {
			...
			if ( field ) {
				if ( ( field.type == 'radio' ) || ( field.type == 'checkbox' ) ) {
					jQuery( field.parentNode ).removeClass( 'hidden' );
				} else {
		...
		for (i=0;i<fieldsToHide.length;i++) {
			...
			if ( field ) {
				if ( ( field.type == 'radio' ) || ( field.type == 'checkbox' ) ) {
					jQuery( field.parentNode ).addClass( 'hidden' );
					jQuery( field ).prop( 'checked', false );
				} else {
					jQuery( field ).addClass( 'hidden' );
