...
class getmypmsproTab extends cbPMSHandler
{
	...
	*/
	...
	public function getDisplayTab( $tab, $user, $ui )
	{
		...
		$return				=	'<form action="' . $_CB_framework->pluginClassUrl( $this->element, true, array( 'id' => (int) $user->get( 'id' ) ) ) . '" method="post" name="quickMsgForm" id="quickMsgForm" class="cb_form m-0 quickMsgForm cbValidation">'
							.		'<div class="card">'
							.			( $showTitle ? '<div class="card-header">' . CBTxt::T( $tab->title ) . '</div>' : null )
							.			'<div class="card-body">';
		...
		if ( $showSubject ) {
			$return			.=				'<div class="cbft_text cbtt_input form-group row no-gutters cb_form_line">'
							.					'<label for="subject" class="col-form-label col-sm-3 pr-sm-2">' . CBTxt::T( 'PM_SUBJECT', 'Subject' ) . '</label>'
		...
		$return				.=				'<div class="cbft_textarea cbtt_textarea form-group row no-gutters cb_form_line">'
							.					( $showSubject ? '<label for="subject" class="col-form-label col-sm-3 pr-sm-2">' . CBTxt::T( 'PM_MESSAGE', 'Message' ) . '</label>' : null )
							...
							.					'<div class="' . ( $showSubject ? 'offset-sm-3 col-sm-9' : 'col-sm-12' ) . '">'
