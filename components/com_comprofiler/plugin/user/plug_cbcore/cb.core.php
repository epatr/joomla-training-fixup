...
//					$html			=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', 'multicheckbox', $value, '', $allValues );
					...
					if ( in_array( $field->type, array( 'multicheckbox', 'radio' ) ) && $field->params->get( 'field_edit_style', 0, GetterInterface::INT ) ) {
						$html		=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', $field->type . 'buttons', $value, ( in_array( $field->type, array( 'multicheckbox', 'multiselect' ) ) ? $this->getDataAttributes( $field, $user, $output, $reason ) : '' ), $allValues );
					} else {
				...
				if ( ( $value != null ) && ( $value != '' ) && ( $value != '0000-00-00 00:00:00' ) && ( $value != '0000-00-00' ) && ( $value != '00:00:00' ) ) {
					...
					switch ( $field->params->get( 'field_display_by', 0, GetterInterface::INT ) ) {
						...
						case 4: // Date
							...
							break;
						case 5: // Custom
							$dateFormat		=	$field->params->get( 'custom_date_format', 'Y-m-d', GetterInterface::STRING );
							$timeFormat		=	$field->params->get( 'custom_time_format', 'H:i:s', GetterInterface::STRING );
			...
			case 'htmledit':
				...
				if ( $reason == 'search' ) {
					...
					if ( $searchBy == 1 ) {
						...
						$minHtml			=	$this->formatFieldValueLayout( $calendars->cbAddCalendar( $minNam, CBTxt::Th( 'UE_SEARCH_FROM', 'Between' ) . ' ' . $this->getFieldTitle( $field, $user, 'text', $reason ), false, $minVal, false, $showTime, $yMin, $yMax, $tooltip, $offset ), $reason, $field, $user );
						$maxHtml			=	$this->formatFieldValueLayout( $calendars->cbAddCalendar( $maxNam, CBTxt::Th( 'UE_SEARCH_TO', 'and' ) . ' ' . $this->getFieldTitle( $field, $user, 'text', $reason ), false, $maxVal, false, $showTime, $yMin, $yMax, $tooltip, $offset ), $reason, $field, $user );
					}
					...
					$return					=	$this->formatFieldValueLayout( $calendars->cbAddCalendar( $field->get( 'name' ), $this->getFieldTitle( $field, $user, 'text', $reason ), $this->_isRequired( $field, $user, $reason ), $value, $this->_isReadOnly( $field, $user, $reason ), $showTime, $yMin, $yMax, $tooltip, $offset, $timeZone ), $reason, $field, $user )
		...
		switch ( $output ) {
			...
			case 'htmledit':
				if ( $reason == 'search' ) {
					...
					$html				=	$this->formatFieldValueLayout( ( $trimmedDescription && $inputDescription ? cbTooltip( $_CB_framework->getUi(), $htmlDescription, $translatedTitle, null, null, $editor, null, 'style="display:block;"' ) : $editor ), $reason, $field, $user, false )
	...
	public function prepareFieldDataSave( &$field, &$user, &$postdata, $reason ) {
			...
			||	( ( isset( $ueConfig['frontend_userparams'] ) ) ? ( $ueConfig['frontend_userparams'] == 1 ) : in_array( $_CB_framework->getCfg( "frontend_userparams" ), array( '1', null) ) ) )
		{
			...
			if ( ( $params != null ) && is_array( $params ) ) {
				$userParams					=	new Registry( $params );
				$value						=	$userParams->asJson();
