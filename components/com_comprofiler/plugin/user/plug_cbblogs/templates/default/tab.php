...
class HTML_cbblogsTab
{
	...
	static function showBlogTab( $rows, $pageNav, $searching, $input, $viewer, $user, /** @noinspection PhpUnusedParameterInspection */ $model, $tab, $plugin )
	{
		...
		$return						.=			'<div class="table-responsive blogsContainer">'
									...
									.							'<th style="width: 24%;" class="d-none d-sm-table-cell">' . CBTxt::T( 'Created' ) . '</th>'
		...
		if ( $rows ) foreach ( $rows as $row ) {
			$return					.=						'<tr>'
									...
									.							'<td style="width: 24%;" class="d-none d-sm-table-cell">' . cbFormatDate( $row->get( 'created' ) ) . '</td>';
			...
			if ( $menuAccess ) {
				...
				$menuAttr			=	cbTooltip( 1, $menuItems, null, 'auto', null, null, null, 'class="btn btn-light border btn-sm blogsMenu" data-cbtooltip-menu="true" data-cbtooltip-classes="qtip-nostyle" data-cbtooltip-open-classes="active"' );
