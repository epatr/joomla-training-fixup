...
class HTML_cbforumsTabCatSubs
{
	...
	static public function showCategorySubscriptions( $rows, $pageNav, $searching, $input, $viewer, $user, $tab, /** @noinspection PhpUnusedParameterInspection */ $plugin )
	{
		...
		$return				=	'<div class="mb-2 forumsCatSubsTab">'
							.		'<form action="' . $_CB_framework->userProfileUrl( $user->id, true, $tab->tabid ) . '" method="post" name="forumCatSubsForm" id="forumCatSubsForm" class="m-0 forumCatSubsForm">';
		...
		if ( $rows ) foreach ( $rows as $row ) {
			$return			.=						'<tr>'
							.							'<td style="width: 99%;"><a href="' . ( isset( $row->category_url ) ? $row->category_url : cbforumsModel::getForumURL( $row->category_id ) ) . '">' . cbforumsClass::cleanPost( $row->category_name ) . '</a></td>'
							.							'<td style="width: 1%;" class="text-right"><a href="javascript: void(0);" onclick="cbjQuery.cbconfirm( \'' . addslashes( CBTxt::T( 'Are you sure you want to delete this subscription?' ) ) . '\' ).done( function() { window.location.href = \'' . addslashes( $_CB_framework->userProfileUrl( $user->id, true, $tab->tabid, 'html', 0, array( 'forums_unsubcat' => $row->id ) ) ) . '\'; })" title="' . htmlspecialchars( CBTxt::T( 'Delete' ) ) . '"><span class="fa fa-trash-o"></span></a></td>'
							.						'</tr>';
		} else {
			$return			.=						'<tr>'
							.							'<td colspan="2">';
