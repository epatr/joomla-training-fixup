		...
		if ( $canvasMenu || $canvasHeader || $canvasMain ) {
			...
			if ( $canvasHeader ) {
				...
				if ( $canvasPhoto || $canvasTitle || $canvasStats ) {
					$return				.=		'<div class="position-relative row no-gutters bg-white border-top' . ( ! $canvasMain ? ' border-bottom' : null ) . ( ! $canvasPhoto ? ' p-2' : null ) . ' cbPosCanvasBottom cbCanvasLayoutBottom">';
		...
		if ( is_array( $this->users ) && ( count( $this->users ) > 0 ) ) foreach ( $this->users as $userIndex => $user ) {
			...
			if ( $columnCount ) {
				...
				if ( $layout == 'grid' ) {
					...
					// If canvas field exists extract canvas layout details so we can correctly format it:
				...
				foreach ( $this->columns as $columnIndex => $column ) {
					...
					if ( $formatting == 'divs' ) {
						if ( $layout == 'grid' ) {
							...
							$return		.=				'<div class="cbCanvasLayoutContent cbCanvasLayoutContent' . ( $columnIndex + 1 ) . ( $column->cssclass ? ' ' . trim( $column->cssclass ) : null ) . '" role="gridcell">' . $cellContent . '</div>';
						} else {
							$return		.=				'<div class="col-sm' . ( $column->size ? '-' . $column->size : null ) . ' border-top p-2 cbColumn' . $column->size . ' cbUserListRowColumn cbUserListRowCol' . ( $columnIndex + 1 ) . ( $column->cssclass ? ' ' . $column->cssclass : null ) . '" role="gridcell">' . $cellContent . '</div>';
						}
					} else {
						$return			.=				'<td class="cbUserListRowColumn cbUserListRowCol' . ( $columnIndex + 1 ) . ( $column->cssclass ? ' ' . trim( $column->cssclass ) : null ) . '">' . $cellContent . '</td>';
					}
				}
