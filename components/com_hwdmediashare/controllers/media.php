...
class hwdMediaShareControllerMedia extends JControllerForm
{
	...
	public function publish()
	{
		...
		else
		{
			...
			else
			{
                ...
                // If a user has just deleted a media, we don't want to redirect them back to a page they are not going to be able to see. 
                if ($task == 'delete')
                {
                        $return = base64_decode($this->input->get('return', null, 'base64'));
                        // If is difficult to determine if the return URL is pointing to the media player page, so...
	...
	public function link()
	{
		...
		else
		{
			...
			if ($model->link($cid))
			{
                                // Surprss success message - use template instead.
			...
			else
			{
                ...
                // $return = base64_decode($this->input->get('return', null, 'base64'));
                // $this->setRedirect($return ? $return : JRoute::_('index.php?option=' . $this->option . '&view=' . $this->view_list, false));
                $this->setRedirect(JRoute::_('index.php?option=com_hwdmediashare&task=mediaform.link&id=' . $this->input->get('id', 0, 'int') . '&return=' . $this->input->get('return', null, 'base64') . '&tmpl=component&complete=1', false));                          
	}        
}
