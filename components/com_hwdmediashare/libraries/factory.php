...
class hwdMediaShareFactory extends JObject
{
	 ...
	 */
        public function getConfig($params = null, $reset = false)
        {
                ...
                // Get the frontend application parameters (when running from site and not running from cli).
                $appParams = !defined('_JCLI') && $app->isSite() ? $app->getParams('com_hwdmediashare') : null;
                
                // Check if the config is already loaded, or if we're resetting.
                if(!$this->_config || $reset || $params)
		{
                        jimport('joomla.filesystem.file');
                        ...
                        // Merge the 'debug' parameter from the Global Configuration (when not running from cli)
                        !defined('_JCLI') ? $this->_params->set('debug', $app->getCfg('debug')) : null;                           
                }
	...
	public static function getUserFromTokenId($tokenId, $userId)
	{
                ...
                // Set token timeout.
                $timeout = JFactory::getDate('-10 hour');
                ...
                $query = $db->getQuery(true)
                        ...
                        ->where('datetime > ' . $db->quote($timeout));
                try
                {
                        $db->setQuery($query);
