...
<div class="btn-toolbar">
  <div class="btn-group">
    <button type="button" class="btn btn-primary" onclick="Joomla.submitbutton('playlists.delete')">
      <i class="icon-checkmark"></i> <?php echo JText::_('COM_HWDMS_REMOVE'); ?>
    </button>
  </div>
  <div class="btn-group">
    <button type="button" class="btn" onclick="Joomla.submitbutton('playlists.publish')">
      <i class="icon-cancel"></i> <?php echo JText::_('COM_HWDMS_PUBLISH'); ?>
    </button>
  </div>
  <div class="btn-group">
    <button type="button" class="btn" onclick="Joomla.submitbutton('playlists.unpublish')">
      <i class="icon-cancel"></i> <?php echo JText::_('COM_HWDMS_UNPUBLISH'); ?>
    </button>
  </div>
  <?php if ($canAdd): ?>
    <a title="<?php echo JText::_('COM_HWDMS_ADD_PLAYLIST'); ?>" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&view=playlistform&layout=edit&return=' . $displayData->return); ?>" class="btn"><i class="icon-plus"></i> <?php echo JText::_('COM_HWDMS_ADD_PLAYLIST'); ?></a>
  <?php endif; ?>      
</div>
<table class="table table-striped table-hover">
  ...
  <tbody>
    ...
    $canEdit = ($user->authorise('core.edit', 'com_hwdmediashare.playlist.'.$item->id) || ($user->authorise('core.edit.own', 'com_hwdmediashare.playlist.'.$item->id) && ($item->created_user_id == $user->id)));
    $canEditState = $user->authorise('core.edit.state', 'com_hwdmediashare.playlist.'.$item->id);
    $canDelete = ($user->authorise('core.delete', 'com_hwdmediashare.playlist.'.$item->id) || ($user->authorise('core.edit.own', 'com_hwdmediashare.playlist.'.$item->id) && ($item->created_user_id == $user->id)));
    ?>
    <tr>
      <td>
        <div class="row-fluid">
          ...
          <div class="span2">
            <div class="media-item<?php echo ($displayData->params->get('list_thumbnail_aspect') == 0 ? ' originalaspect' : ''); ?><?php echo ($displayData->params->get('list_tooltip_location') > '2' ? ' hasTooltip' : ''); ?>" title="<?php echo ($displayData->params->get('list_tooltip_location') > '1' ? JHtml::tooltipText($item->title, ($displayData->params->get('list_tooltip_contents') != '0' ? JHtml::_('string.truncate', $item->description, $displayData->params->get('list_desc_truncate'), false, false) : '')) : $item->title); ?>">
              ...
              <div class="media-item-format-2">
                <img src="<?php echo JHtml::_('hwdicon.overlay', 4, $item); ?>" alt="<?php echo JText::_('COM_HWDMS_PLAYLIST'); ?>" />
              </div>
              <?php endif; ?>
              <?php if ($displayData->params->get('list_link_thumbnails') == 1) :?><a href="<?php echo JRoute::_(hwdMediaShareHelperRoute::getPlaylistRoute($item->slug)); ?>"><?php endif; ?>
          ...
          <div class="span<?php echo $displayData->params->get('list_meta_thumbnail') != '0' ? 9: 11; ?>">
            ...
            <?php if ($displayData->params->get('list_meta_title') != '0') :?>
              <h<?php echo $displayData->params->get('list_item_heading'); ?> class="contentheading<?php echo ($displayData->params->get('list_tooltip_location') > '1' ? ' hasTooltip' : ''); ?>" title="<?php echo ($displayData->params->get('list_tooltip_location') > '1' ? JHtml::tooltipText($item->title, ($displayData->params->get('list_tooltip_contents') != '0' ? JHtml::_('string.truncate', $item->description, $displayData->params->get('list_desc_truncate'), false, false) : '')) : $item->title); ?>">
                <?php if ($displayData->params->get('list_link_titles') == 1) :?><a href="<?php echo JRoute::_(hwdMediaShareHelperRoute::getPlaylistRoute($item->slug)); ?>"><?php endif; ?>
