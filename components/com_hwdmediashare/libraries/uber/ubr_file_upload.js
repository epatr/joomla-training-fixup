...
function checkFileNameFormat(){
	...
	for(var i = 0; i < upload_range; i++){
		if(document.adminForm.elements['upfile_' + i].value != ""){
			var string = document.adminForm.elements['upfile_' + i].value;
...
function checkAllowFileExtensions(){
	...
	for(var i = 0; i < upload_range; i++){
		if(document.adminForm.elements['upfile_' + i].value != ""){
			if(!document.adminForm.elements['upfile_' + i].value.match(allow_extensions)){
				var string = document.adminForm.elements['upfile_' + i].value;
				var num_of_last_slash = string.lastIndexOf("\\");
...
function checkDisallowFileExtensions(){
	...
	for(var i = 0; i < upload_range; i++){
		if(document.adminForm.elements['upfile_' + i].value != ""){
			if(document.adminForm.elements['upfile_' + i].value.match(disallow_extensions)){
				var string = document.adminForm.elements['upfile_' + i].value;
				var num_of_last_slash = string.lastIndexOf("\\");
...
function checkNullFileCount(){
	...
	for(var i = 0; i < upload_range; i++){
		if(document.adminForm.elements['upfile_' + i].value == ""){ null_file_count++; }
	}
...
function hideProgressBar(){ document.getElementById('progress_bar').addClass('hide'); }
...
function iniFilePage(){
	...
	for(var i = 0; i < upload_range; i++){
		document.adminForm.elements['upfile_' + i].disabled = false;
		document.adminForm.elements['upfile_' + i].value = "";
	}

	document.getElementById('upload_button').disabled = false;
	document.getElementById('progress_bar').addClass('hide');
...
function linkUpload(){
	...
	{
		//document.getElementById("adminForm").target = "upload_iframe";
	}
	...
	if(show_files_uploaded){
		...
		for(var i = 0; i < upload_range; i++){
			if(document.adminForm.elements['upfile_' + i].value != ""){ total_uploads++; }
		}
...
function startProgressBar(upload_id, upload_size, start_time){
	...
	get_data_loop = true;
	document.getElementById('progress_bar').removeClass('hide');
