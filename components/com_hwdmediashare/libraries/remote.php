...
{        
                ...
                foreach($urls as $url)
                {
                        ...
                        else
                        {
                                ...
                                for($i = 1; $i <= count($parts) - 2; $i++)
                                {
                                        ...
                                        $remotePluginClass = $this->getRemotePluginClass($lookup);
                                        $remotePluginPath = $this->getRemotePluginPath($lookup);
		...
		foreach ($files as $file)
		{
                        ...
                        if ($ext_id > 0)
                        {
                                ...
                                else
                                {
                                        ...
                                        $post['published']             = 1;
                                        $post['featured']              = 0;
                                        $post['access']                = $config->get('default_access', 1);
                                        $post['download']              = $config->get('default_download', 1);
                                        $post['created_user_id']       = $user->id;
                                        $post['created_user_id_alias'] = '';
                                        $post['created']               = $date->toSql();
	 ...
	 */
	public function clean($string, $truncate = 5120, $whiteListTags = array())
	{
                // We will filter input from the buffer.
                $filter = JFilterInput::getInstance($whiteListTags);

                $string = (string) nl2br($string);
