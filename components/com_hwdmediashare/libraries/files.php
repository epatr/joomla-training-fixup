...
class hwdMediaShareFiles extends JObject
{
        ...
        public static function getExtension($item, $fileType)
        {
                ...
                switch ($fileType) 
                {
                        case 1:
                                ...
                                if (!isset(self::$loadedExtensions[$item->ext_id])) 
                                {
                                        $query = $db->getQuery(true)
                                                ->select('ext')
                                                ->from('#__hwdms_ext')
                                        ...
                                        {                
                                                $db->setQuery($query);
                                                self::$loadedExtensions[$item->ext_id] = $db->loadResult();
                                        }
                                        catch (Exception $e)
                                        {
                                                $this->setError($e->getMessage());
                                ...
                                return self::$loadedExtensions[$item->ext_id];  
                        break;
                        case 2:
                        case 3:
                        case 4:
                        ...
                        case 10:
                                ...
                                if (!isset(self::$loadedExtensions[$item->thumbnail_ext_id])) 
                                {
                                        $query = $db->getQuery(true)
                                                ->select('ext')
                                                ->from('#__hwdms_ext')
                                        ...
                                        {                
                                                $db->setQuery($query);
                                                self::$loadedExtensions[$item->thumbnail_ext_id] = $db->loadResult();
                                        }
                                        catch (Exception $e)
                                        {
                                                $this->setError($e->getMessage());
                                ...
                                return self::$loadedExtensions[$item->thumbnail_ext_id];  
                        break;
                        case 11:
                        case 12:
                        case 13:
	 ...
	 * Method to check if a file has been generated and return file data.
         ...
         * @return  mixed   The mp3 file object, false on fail.
	 */
	public function getFileByBasename($basename)
	{
                // Create a new query object.
                $db = JFactory::getDBO();
                $query = $db->getQuery(true);
                ...
                // Select the required fields from the table.
                $query->select('f.*, m.id as media_id, m.title as media_title, m.type as media_type');

                // From the files table.
                $query->from('#__hwdms_files as f');
                $query->join('LEFT', '#__hwdms_media AS m ON m.id = f.element_id');              
                $query->where($db->quoteName('basename').' = '.$db->quote($basename));
