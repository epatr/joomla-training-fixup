		...
		foreach($custom->fields as $group => $fields)
		{
			foreach($fields as $field)
			{
				...
				$filter = $this->getFilter($field);
                                ...
                                // We want to avoid overwriting existing field values with empty values 
                                // when no data is passed about a field, so we perform an additional
                                // check in the posted data to see if the field exists.
                                if (!isset($_POST['field' . $field->id])) continue;
