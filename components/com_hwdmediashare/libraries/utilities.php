...
class hwdMediaShareUtilities extends JObject
{
	 ...
	 * @param   string  $margin       Flag for margin.
	 * @return  string  The rendered notice.
	 */
	public function printNotice($statement, $description = '', $type = 'info', $margin = false)
	{ 
                ob_start();
                ?>
                <div class="clearfix">
                  <div class="alert alert-<?php echo $type; ?><?php echo $margin ? ' margin10' : ''; ?>">
	...
	{ 
                ...
                <div id="hwd-container" class="hwd-modal"> <a name="top" id="top"></a>
                  <h2 class="media-modal-title"><?php echo JText::_($heading); ?></h2>
	...
	public function autoCreateChannel($pk)
	{
		...
		$pk = (int) $pk;
			...
			return false;
                ...
                // Check for valid asset_id otherwise permission checking will fail.
                if (isset($exists['asset_id']) && $exists['asset_id'] == 0)
                {   
                        JTable::addIncludePath(JPATH_ADMINISTRATOR.'/components/com_hwdmediashare/tables');
                        $table = JTable::getInstance('Channel', 'hwdMediaShareTable');

                        // Save data to the database.
                        if (!$table->save($exists))
                        {
