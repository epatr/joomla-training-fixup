...
class hwdMediaShareCategory extends JObject
{
	 ...
	 * @var         array
	 */ 
	protected static $loadedCategories = array();
	 ...
	 */ 
        ...
        {
                $item->id = (int) $item->id;
                $hash = md5($this->elementType . ':' . $item->id);
		if (!isset(self::$loadedCategories[$hash])) 
                {
                        // Initialiase variables.
                        $db = JFactory::getDBO();
                        $user	= JFactory::getUser();
			...
			self::$loadedCategories[$hash] = $result;
		}

		return self::$loadedCategories[$hash];             
        }
