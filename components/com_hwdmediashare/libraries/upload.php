...
class hwdMediaShareUpload extends JObject
{
	...
	public function process($upload)
	{
                ...
                if (empty($file['tmp_name']) || !is_uploaded_file($file['tmp_name']))
                {
                        ...
                        {
                                $this->setError(JText::_('COM_HWDMS_ERROR_PHP_UPLOAD_ERROR_' . $file['error']));
                                return false; 
                        }
                        else
                        {
...
/////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                // We want to run this data through Joomla filters, so we set the $_REQUEST associative array.
                $_REQUEST['id']          = isset($_POST_DATA['jform_id_']) ? $_POST_DATA['jform_id_'] : false;
                $_REQUEST['title']       = isset($_POST_DATA['jform_title_']) ? $_POST_DATA['jform_title_'] : false;
                $_REQUEST['alias']       = isset($_POST_DATA['jform_alias_']) ? $_POST_DATA['jform_alias_'] : false;
                $_REQUEST['description'] = isset($_POST_DATA['jform_description_']) ? $_POST_DATA['jform_description_'] : false;
                $_REQUEST['catid']       = isset($_POST_DATA['jform_catid___']) ? array($_POST_DATA['jform_catid___']) : false;
                $_REQUEST['tags']        = isset($_POST_DATA['jform_tags___']) ? array($_POST_DATA['jform_tags___']) : false;
                $_REQUEST['category_id'] = isset($_POST_DATA['jform_category_id_']) ? $_POST_DATA['jform_category_id_'] : false;
                $_REQUEST['album_id']    = isset($_POST_DATA['jform_album_id_']) ? $_POST_DATA['jform_album_id_'] : false;
                $_REQUEST['playlist_id'] = isset($_POST_DATA['jform_playlist_id_']) ? $_POST_DATA['jform_playlist_id_'] : false;
                $_REQUEST['group_id']    = isset($_POST_DATA['jform_group_id_']) ? $_POST_DATA['jform_group_id_'] : false;

                // Get associations from ubr_upload and assign them to the jform array.
                $data = array();
                $data['id'] = $app->input->get('id', '', 'integer');
                ...
                // Check if we are replacing an existing media item.
                if (isset($data['id']) && $data['id'] > 0 && $app->isAdmin() && $user->authorise('core.edit', 'com_hwdmediashare'))
                {
                        // Unset the category element to avoid removing any assigned categories.
                        unset($data['catid']);
                }
                ...
                foreach($_FILE_DATA as $arrayKey => $slot)
                {
                        ...
                        // Check the file slot has been defined as expected.
                        if (!isset($slot->name))
                        {
				...
				if (!$key = $utilities->generateKey(1))
				{
					...
					return false;
                        ...
                        {          
                                ...
                                $post['media_type']             = '';
                                $post['key']                    = $key;
                                $post['title']                  = isset($data['title']) && !empty($data['title']) ? $data['title'] : hwdMediaShareUpload::removeExtension($_POST_DATA[$slot->slot]);
                                $post['alias']                  = isset($data['alias']) && !empty($data['alias']) ? JFilterOutput::stringURLSafe($data['alias']) : JFilterOutput::stringURLSafe($post['title']);
                                $post['description']            = isset($data['description']) ? $data['description'] : '';
                                $post['type']                   = 1; // Local
                                $post['source']                 = '';
                                $post['storage']                = '';
	...
	public function processThumbnail($item)
	{
                ...
                if (isset($object) && is_object($object))
                {
                        ...
                        // Add file record.
                        if (isset($object->thumbnail_ext_id)) 
                        {
                                $item->thumbnail_ext_id = (int) $object->thumbnail_ext_id;
                                $HWDfiles = hwdMediaShareFiles::getInstance();
                                $HWDfiles->elementType = $this->elementType;
        ...
        public static function getFlashUploadURI()
	{
        ...
        /**
	 * Method to get maximum upload size (in bytes) for specified upload method.
         * 
         * @access  public
         * @static
        ...
        public static function getMaximumUploadSize($method = null)
	{
                ...
                // Generate the maximum PHP upload limit.
                $post_max_size = hwdMediaShareUpload::parseSize(ini_get('post_max_size'));
                $upload_max_filesize = hwdMediaShareUpload::parseSize(ini_get('upload_max_filesize'));

                switch ($method) 
                {
                        case 'standard':                        
                                $maxUpload = intval($config->get('max_upload_filesize', 30) * 1024 * 1024);    
                                $maxPhpUpload = min($post_max_size, $upload_max_filesize);
			...
			// JArrayHelper::toInteger($tags);
                ...
                if (isset($data['category_id']))
                {
                        ...
                        {            
                                ...
                                $HWDcategory->saveIndividual($catid, $media->id);
                        }
                }
	...
	public static function addProcesses($media)
	{
                ...
                if ($config->get('process_auto') == 1)
                {
                        ...
                        {
                                // Surpress the error if exec() is disabled. 
                                // Trailing ampersand tells the terminal not to wait for a response.
                        ...
                        else 
                        {
                                ...
                                // $oExec = $WshShell->Run('"' . $config->get('path_php') . '" "' . $cli . '" process ' . implode(" ", $processes), 0, false);

                                // Adopted method.                            
                                pclose(popen("start /B ". $config->get('path_php')." " . $cli . " process ".implode(" ", $processes), "r"));  
                        }
                }
	}
	...
	public static function checkLimits()
	{
                ...
                if ($config->get('enable_limits'))
                {
                        ...
                        {
                                $group_limit = $limits[$values[$config->get('enable_limits')]][$group];
                                if ($group_limit > 0)
                                {
	...
	/**
	 * Method to parse a size (from PHP configuration).
         * 
         * @access  public
         * @static
