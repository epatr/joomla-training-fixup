...
/**
 * This template is used to generate the body of the email notification sent when new albums
 * are added but require approval. The album object is contained in $album.
 */
...
// Load the author.
$author = JFactory::getUser($album->created_user_id);

$link_site = $utilities->relToAbs(JRoute::_(hwdMediaShareHelperRoute::getAlbumRoute($album->id)));
$link_admin = JURI::root() . "administrator/index.php?option=com_hwdmediashare&task=album.edit&id=" . $album->id;
$link_pending = JURI::root() . "administrator/index.php?option=com_hwdmediashare&view=albums&filter[status]=2";
?>
...
<html>
  ...
  <body>
    <p><?php echo JText::sprintf('COM_HWDMS_EMAIL_NEWALBUM_INTRO_PENDING', '<a href="' . JURI::root() . '" target="_blank">' . $app->getCfg('sitename') . '</a>'); ?></p>
    <h1><a href="<?php echo $link_site; ?>" target="_blank"><?php echo $album->title; ?></a></h1>
