...
class JDocumentRendererMRSS extends JDocumentRenderer
{
	/**
	 * Renderer mime type.
	 *
	...
	/**
	 * Render the Media RSS Specification feed.
	 *
         * @access  public
	...
	public function render($name = 'mrss', $params = null, $content = null)
	{
		...
		{
			//if (count($data->items[$i]->mediafiles) == 0) continue;                    
			...
			if ($data->items[$i]->_media->type == 7 && $data->items[$i]->_media->source)
                        {
                                ...
                                $feed.= '			<media:content ';
                                $feed.= 'url="' . str_replace('&','&amp;',$data->items[$i]->_media->source) . '" ';
                                $feed.= 'type="' . hwdMediaShareDocuments::getContentType(JFile::getExt($data->items[$i]->_media->source)) . '" ';
                                $feed.= 'medium="video">';
                                $feed.= "</media:content>\n";  
                        } 

                        // Add custom remote media locations.                        
			if ($data->items[$i]->_media->type == 2 && $data->items[$i]->_media->source && !filter_var($data->items[$i]->_media->source, FILTER_VALIDATE_URL) === false)
                        {
                                $sourceHost = str_ireplace('www.', '', parse_url($data->items[$i]->_media->source, PHP_URL_HOST));
                                if (in_array($sourceHost, array('youtube.com')))
                                {
                                        hwdMediaShareFactory::load('documents');
                                        $feed.= '			<jwplayer:source ';
                                        $feed.= 'file="' . str_replace('&','&amp;',$data->items[$i]->_media->source) . '" ';
                                        $feed.= 'type="youtube">';
                                        $feed.= "</jwplayer:source>\n";  
                                }
                        } 
                        ...
                        if ($data->items[$i]->image)
                        {
                                ...
                                $feed.= 'url="' . str_replace('&','&amp;',$this->_relToAbs($data->items[$i]->image)) . '">';
                                $feed.= "</media:thumbnail>\n";  
                        }
