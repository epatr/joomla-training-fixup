...
class hwdMediaShareThumbnails extends JObject
{
        ...
        public static function getMediaThumbnail($item)
	{
                ...
                {
                        // Avoid loading mixed (insecure) display content on a secure page.
                        if (JURI::getInstance()->getScheme() == 'https' && parse_url($item->thumbnail, PHP_URL_SCHEME) == 'http')
                        {
                                // Get thumbnail host (without subdomains).
                                $host = parse_url($item->thumbnail, PHP_URL_HOST);
                                $host = explode('.', $host); 
                                $host = array_reverse($host);
                                $host = $host[1] . '.' . $host[0];
                                
                                // Build array of hosts which support secure content.
                                $supported = array(
                                    'ytimg.com',
                                    'dmcdn.net'
                                );
                ...
                if (!isset($item->id) || !isset($item->key) || !isset($item->ext_id))
		{
                ...
                if ($item->type == 5)
		{
                        if (empty($item->storage)) $item->storage = $config->get('cdn','cdn_amazons3');
                        ...
                        if (file_exists($pluginPath))
                        {
                                ...
                                if (method_exists($HWDcdn, 'getThumbnail'))
                                {
                                        if ($file = $HWDcdn->getThumbnail($item))
                                        {
                                                return $file->url;
                                        } 
                                }
                ...
                // Check if original is a native image format.
        ...
        public static function getPlaylistThumbnail($item)
	{
                ...
                // Check for a custom thumbnail.
                if ($custom = hwdMediaShareThumbnails::getElementThumbnail($item, 4))
                {
                        return $custom;
                }
	 ...
	 */ 
		...
		// Initialiase variables.
                ...
                {           
                        ...
                        if (!empty($avatar) && file_exists(JPATH_ROOT.'/images/comprofiler/tn'.$avatar))
                        {
                                return JURI::root(true).'/images/comprofiler/tn'.$avatar;
                        }
                        elseif (!empty($avatar) && file_exists(JPATH_ROOT.'/images/comprofiler/'.$avatar))
                        {
                                return JURI::root(true).'/images/comprofiler/'.$avatar;
                        } 
                }
        ...
        public static function getVideoPreview($item)
	{
                ...
                if ($item->type == 5 && $item->storage)
		{
                        ...
                        if (file_exists($pluginPath))
                        {
                                ...
                                {
                                        if ($file = $HWDcdn->getFile($item, $fileTypes))
                                        {
                                                return $file->url;
                                        }    
                                }
