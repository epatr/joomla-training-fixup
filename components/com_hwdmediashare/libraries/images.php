...
class hwdMediaShareImages extends JObject
{
	...
	public static function display($item)
	{
                ...
                {
                        list($iwidth, $iheight, $itype, $iattr) = getimagesize($image->path);
                        $width = min(intval($iwidth), intval($config->get('mediaitem_width') ? $config->get('mediaitem_width') : $config->get('mediaitem_size')));

                        ob_start(); ?>
                        <img src="<?php echo $image->url; ?>" border="0" alt="<?php echo $utilities->escape($item->title); ?>" style="width:<?php echo $width; ?>px;" />
                        <?php
	...
	public static function getJpg($item)
	{
                ...
                if ($item->type == 5 && $item->storage)
		{
                ...
                // Check if original is native image format, with smaller dimensions than requested. This
                // allows us to show the original instead improving the display.
                ...
                list($iwidth, $iheight, $itype, $iattr) = getimagesize($path);
                if (file_exists($path) && hwdMediaShareImages::isNativeImage($ext) && ($size > $iwidth || $size > $iheight))
                {
                        // Create file object.
                        $file = new JObject;
                        $file->local = true;
	...
	public static function getMeta($item)
	{
                ...
                // Check original file exists and is supported.
                if (file_exists($pathSource) && filesize($pathSource) > 0 && in_array($extSource, array('jpg', 'jpeg', 'tiff')))
                {
                        try
                        {
                        ...
                        {
                                if (is_array($exif) && array_key_exists($key, $exif))
                                {
	 ...
	 * Method to resize an image.
         * 
         * @access  public
         * @param   object   $path  The path of the image.
         * @param   integer  $size  The size of the image.
	...
	public function resizeImage($path, $size)
	{
                ...
                if (file_exists($pathDest) && filesize($pathDest) > 0)
                {
                        // Copy the resized image to replace the original.
                        if (JFile::copy($pathDest, $pathSource))
                        {
                                JFile::delete($pathDest);
                                return true;
                ...
                try
                {
                        ...
                        if(substr(PHP_OS, 0, 3) == "WIN")
                        {
                                $input = "\"".$config->get('path_imagemagick', '/usr/bin/convert')."\" -verbose -resize $wxh $pathSource $pathDest 2>&1";
                                exec($input, $output);
                        }
                        else
                        {
