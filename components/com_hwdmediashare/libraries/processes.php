...
class hwdMediaShareProcesses extends JObject
{
	 ...
	 */
	public function countQueue($media_id = 0)
	{
                // Initialise variables. 
                $db = JFactory::getDBO();
                ...
                if ($media_id)
                { 
                        $query->where('media_id = ' . $db->quote($media_id));
                }
	...
	public function addLog($item)
	{
                ...
                $output = JHtml::_('string.truncate', $output, 5120, false, true);
	...
	public function process21($process)
	{
                ...
                {
                        $HWDvideos = hwdMediaShareVideos::getInstance();
                        $HWDvideos->checkMoovAtom($process, $fileType);
                }
	} 
