...
require_once JPATH_ADMINISTRATOR.'/components/com_hwdmediashare/models/channel.php';

class hwdMediaShareModelChannelForm extends hwdMediaShareModelChannel
{
	/**
	...
	{
		// Initialise variables.
		$pk = (int) (!empty($pk)) ? $pk : $this->getState('channel.id');
		...
		$user	= JFactory::getUser();
		$userId	= $user->get('id');
		$asset	= 'com_hwdmediashare.channel.' . $value->id;
		...
		{                       
                        ...
                        {
                                $value->title = $config->get('author') == 0 ? JFactory::getUser($value->id)->name : JFactory::getUser($value->id)->username;
                        }    
                        
                        // Add the tags.
                        $value->tags = new JHelperTags;
                        $value->tags->getTagIds($value->id, 'com_hwdmediashare.channel');
