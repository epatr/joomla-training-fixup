...
class hwdMediaShareModelMediaItem extends JModelItem
{
	...
	public function getItem($pk = null)
	{
                ...
                {                                    
                        if ($option == 'com_hwdmediashare' && $view == 'mediaitem') 
                        {
                                if ($layout == 'modal') 
                                {
                                        ...
                                        // Redirect top frame
                                    	echo "<script>window.top.location.href='" . ($config->get('no_access_redirect') > 0 ? ContentHelperRoute::getArticleRoute($config->get('no_access_redirect')) : 'index.php') . "';</script>\n";
		...
		if ($pk)
		{
                        ...
                        {   
                                $author = JFactory::getUser($this->_media->created_user_id);
                                $this->_media->author = (!empty($this->_media->created_user_id_alias) ? $this->_media->created_user_id_alias : ($config->get('author') == 0 ? $author->name : $author->username));
                        }
                        else
                        {
	...
	public function like($pk = 0, $value = 1)
	{
			...
			return false;
                ...
                if ($user->id)
                {
                        ...
                        if ($liked)
                        {  
                                $this->setError(JText::_('COM_HWDMS_ERROR_ALREADY_LIKED_MEDIA' . $value));
                                return false;
                        } 
                }
                else
                {
                        ...
                        $liked = $app->getUserState('media.media-like-' . $pk);
                        if ($liked)
                        {  
                                $this->setError(JText::_('COM_HWDMS_ERROR_ALREADY_LIKED_MEDIA' . $value));
                                return false;
                ...
                // Log the like/dislike in database table.
                JTable::addIncludePath(JPATH_ADMINISTRATOR.'/components/com_hwdmediashare/tables');
                $table_likes = JTable::getInstance('Likes', 'hwdMediaShareTable');    
                ...
                if (!$table_likes->save($object))
                {
                        $this->setError($table_likes->getError());
                        return false;
                }                

                // Store in state.
                $app->setUserState('media.media-like-' . $pk, 1);
	...
	public function publish($pks, $value = 0)
	{
		...
		foreach ($pks as $i => $id)
		{
			...
			{
                                if ($user->authorise('core.edit.own', 'com_hwdmediashare.media.'. (int) $id))
                                {                                 
                                        $media = $this->getItem($id);
                                        if (!$media || $media->created_user_id != $user->id)
                                        {
