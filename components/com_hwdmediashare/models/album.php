...
class hwdMediaShareModelAlbum extends JModelList
{
	...
	public function getAlbum($pk = null)
	{
		...
		if ($pk)
		{
                        ...
                        {   
                                $author = JFactory::getUser($this->_album->created_user_id);
                                $this->_album->author = (!empty($this->_album->created_user_id_alias) ? $this->_album->created_user_id_alias : ($config->get('author') == 0 ? $author->name : $author->username));
                        }
                        else
                        {
