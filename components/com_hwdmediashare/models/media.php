...
class hwdMediaShareModelMedia extends JModelList
{
	 ...
	 */
		...
		$query->select(
			$this->getState(
				...
				'a.id, a.ext_id, a.key, a.title, a.alias, a.description, a.type, a.source, a.storage, a.duration,' .
                                'a.streamer, a.file, a.embed_code, a.thumbnail, a.thumbnail_ext_id, a.location, a.likes, a.dislikes, a.status, a.published, a.featured,' .
                                'a.checked_out, a.checked_out_time, a.access, a.params, a.ordering, a.created_user_id, a.created_user_id_alias,' .
                                'a.created, a.publish_up, a.publish_down, a.modified, a.hits, a.language'
			)
		);
                // a.params - needed to check access restrictions when loading lightboxes from listings
                // a.embed_code - needed when loading a single item from mod_media_item
			...
			if (stripos($search, 'id:') === 0)
				...
				$query->where('a.id = '.(int) substr($search, 3));
                        ...
                        else
                        {
                                ...
                                {
                                        $query->where('MATCH(a.title, a.description) AGAINST (' . $db->quote($search) . ')');
                                        
                                        // If we are viewing a media item, then we don't want to include this in the search results
                                        if ($app->input->get('option') == 'com_hwdmediashare' && $app->input->get('view') == 'mediaitem' && $app->input->getInt('id') > 0)
                                        {
                                                $query->where('a.id <> ' . $db->quote($app->input->getInt('id')));
                                        }
                                        
                                        // Remove ordering to order by relavency.
                                        $this->state->set('list.ordering', '');
                                        $this->state->set('list.direction', '');
                                }
                                else
                                {
		...
		if (is_numeric($mediaType) && $mediaType > 0)
                {
		...
		if ($favourtiesId = $this->getState('filter.favourites_id'))
                ...
                // Filter by linked media (allowing the display of media linked to another media).
		$mediaId = $this->getState('filter.media_id');
                if ($mediaId > 0)
                {
                        $query->join('LEFT', '#__hwdms_media_map AS mmap ON mmap.media_id_1 = a.id OR mmap.media_id_2 = a.id');
                        $query->where('(mmap.media_id_1 = ' . $db->quote($mediaId) . ' OR mmap.media_id_2 = ' . $db->quote($mediaId) . ')');
                }
                
                // Filter by linked responses (allowing the display of responses to a media).
                $responseId = $this->getState('filter.response_id');
                if ($responseId > 0)
                {
                        $query->join('LEFT', '#__hwdms_response_map AS rmap ON rmap.response_id = a.id');
                        $query->where('rmap.media_id = ' . $db->quote($responseId));                  
                }
		...
		if ((!$user->authorise('core.edit.state', 'com_hwdmediashare')) && (!$user->authorise('core.edit', 'com_hwdmediashare')))
			...
			$this->setState('filter.status',	array(0,1,2,3));
                ...
                if ($this->context == 'com_hwdmediashare.media')
                {
                        ...
                        if ($config->get('list_media_type')) 
                        {
                                $mediaType = $config->get('list_media_type');
                         ...
                         */ 
                        if ($app->input->get('filter_search', '', 'string'))                $this->setState('filter.search', $app->input->get('filter_search', '', 'string'));
                        if ($app->input->get('filter_media_type', '', 'integer') > 0)       $this->setState('filter.media_type', $app->input->get('filter_media_type', '', 'integer'));
                        if ($app->input->get('filter_featured', '', 'string'))              $this->setState('filter.featured', $app->input->get('filter_featured', '', 'string'));
