...
class hwdMediaShareModelAlbums extends JModelList
{
	...
	public function getItems()
	{
		...
		{            
                        for ($x = 0, $count = count($items); $x < $count; $x++)
                        {
                                ...
                                {
                                        $items[$x]->nummedia = $cache->call(array('hwdMediaShareModelAlbums', 'getMediaCount'), $items[$x]);
                                }
                                else
                                {
                                        $items[$x]->nummedia = hwdMediaShareModelAlbums::getMediaCount($items[$x]);
                                }
                        }
                }
	...
	public function populateState($ordering = null, $direction = null)
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_hwdmediashare')) && (!$user->authorise('core.edit', 'com_hwdmediashare')))
			...
			$this->setState('filter.status',	array(0,1,2,3));
                ...
                // Only set these states when in the com_hwdmediashare.albums context.
                if ($this->context == 'com_hwdmediashare.albums')
                {                    
                        ...
                        $featured = $this->getUserStateFromRequest('albums.show_featured', 'show_featured', $config->get('show_featured', 'show'), 'word', false);
                        if (!in_array(strtolower($featured), array('show', 'hide', 'only'))) $display = 'show';
                        $this->setState('albums.show_featured', $featured);
                        $this->setState('filter.featured', $featured);

                        // Check for list inputs and set default values.
                        $orderingFull = $config->get('list_order_album', 'a.created DESC');
	 ...
	 * @return  mixed  An array of data items on success, false on failure.
	 */
	public static function getMediaCount($item)
	{
                ...
                $model->context = 'com_hwdmediashare.albums';
                $model->populateState();
