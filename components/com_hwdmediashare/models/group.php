...
class hwdMediaShareModelGroup extends JModelList
{
	...
	public function getGroup($pk = null)
	{
		...
		if ($pk)
		{
                        ...
                        {   
                                $author = JFactory::getUser($this->_group->created_user_id);
                                $this->_group->author = (!empty($this->_group->created_user_id_alias) ? $this->_group->created_user_id_alias : ($config->get('author') == 0 ? $author->name : $author->username));
                        }
                        else
                        {
                        ...
                        // Add map.
                        if ($config->get('groupitem_media_map'))
                        {
                                $doc->addScript('https://maps.googleapis.com/maps/api/js?v=3.exp&signed_in=true&key=' . $config->get('google_maps_api_v3_key', 'AIzaSyAyxMefjQ8yajKyvBTMD9KJq-M6n80CGsw'));
...
function initialize() {
  ...
  var map = new google.maps.Map(document.getElementById(\'media-group-map-canvas\'), mapOptions);

  var georssLayer = new google.maps.KmlLayer({
    url: \'' . JURI::root() . 'index.php?option=com_hwdmediashare&view=media&format=feed&type=rssgeo&filter_group_id=' . $this->_group->id . '\'
  });
