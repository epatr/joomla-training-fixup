...
class hwdMediaShareModelChannel extends JModelList
{
	...
	public function getChannel($pk = null)
	{
		...
		if ($pk)
		{
                        ...
                        {   
                                $author = JFactory::getUser($this->_channel->id);
                                $this->_channel->title = $config->get('author') == 0 ? $author->name : $author->username;
                        }
	...
	{       
                JModelLegacy::addIncludePath(JPATH_ROOT.'/components/com_hwdmediashare/models');
                $this->_model = JModelLegacy::getInstance('Channels', 'hwdMediaShareModel', array('ignore_request' => true));
                $this->_model->populateState();
	...
	public function populateState($ordering = null, $direction = null)
	{
		...
		if ($app->input->get('option', '', 'word') == 'com_hwdmediashare' && $app->input->get('view', '', 'word') == 'account')
                ...
                elseif ($app->input->get('option', '', 'word') == 'com_hwdmediashare' && $app->input->get('view', '', 'word') == 'channel' && (!$app->input->getInt('id') && $user->id))
                {
                        $id = (int) $user->id;
                }
                else
