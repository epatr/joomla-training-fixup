...
class hwdMediaShareViewUpload extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$this->params = $this->state->params;
                ...
                if (!$user->authorise('hwdmediashare.upload', 'com_hwdmediashare') && !$user->authorise('hwdmediashare.import', 'com_hwdmediashare'))
                {
                        ...
                        $app->redirect($this->config->get('no_access_redirect') > 0 ? ContentHelperRoute::getArticleRoute($this->config->get('no_access_redirect')) : ($user->id ? hwdMediaShareHelperRoute::getMediaRoute() : 'index.php?option=com_users&view=login&return=' . base64_encode(JFactory::getURI()->toString())));
                }
		...
		if ($this->method != 'remote' && ($this->config->get('enable_uploads_file') == 0 || !$user->authorise('hwdmediashare.upload', 'com_hwdmediashare')) && ($this->config->get('enable_uploads_remote') == 1 && $user->authorise('hwdmediashare.import', 'com_hwdmediashare'))) 
		{
			$this->method = 'remote';
		}
	...
	protected function getPlatformUploadForm()
	{
                ...
                if (file_exists($pluginPath))
                {
                        ...
                        if ($form = $HWDplatform->getSiteUploadForm($this))
                        {
                                return $form;
                        }
                        else
