...
$share_url            = hwdMediaShareMedia::getPermalink($this->item->id, false);
...
// Define the URL where the facebook dialogue redirects after sharing. It is good to redirect to a local HTML file that will 
// close the window through Javascript for example, with this code:
// <html><body><script>window.close()</script></body></html>
$fb_redirect          = file_exists(JPATH_ROOT.'/jsclose.html') ? JURI::root(false).'jsclose.html' : 'http://facebook.com/';  
?>
<form action="<?php echo htmlspecialchars(JFactory::getURI()->toString()); ?>" method="post" name="adminForm" id="adminForm">
  <div id="hwd-container" class="hwd-modal <?php echo $this->pageclass_sfx;?>"> <a name="top" id="top"></a>
    ...
    <div class="row-fluid share-panel">
      <div class="span6">
        ...
        <div class="row-fluid share-url">
          <i class="icon-link"></i>
          <input type="text" title="<?php echo JText::_('COM_HWDMS_PERMALINK'); ?>" value="<?php echo $share_permalink; ?>" name="share_url" class="share-url-input span12">
        </div>
      </div>    
      <div class="span6">
        <label><?php echo JText::_('COM_HWDMS_SOCIAL'); ?></label>
        <div class="social-icon pull-left">
          <a href="https://www.facebook.com/dialog/share?app_id=<?php echo $this->params->get('facebook_appid', '121857621243659'); ?>&display=popup&href=<?php echo urlencode($share_url); ?>&display=popup&redirect_uri=<?php echo urlencode($fb_redirect); ?>" onclick="javascript:window.open(this.href,'','menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=236,width=561');return false;">
        ...
        <div class="social-icon pull-left">
          <script type="text/javascript" src="<?php echo JURI::getInstance()->getScheme(); ?>://platform.twitter.com/widgets.js"></script>
        ...
        <div class="social-icon pull-left">
          <a href="//www.pinterest.com/pin/create/button/?url=<?php echo urlencode($share_url); ?>&description=<?php echo urlencode($share_description); ?>&media=<?php echo urlencode($share_thumbnail); ?>" data-pin-do="buttonPin" data-pin-config="above" onclick="javascript:window.open(this.href,'','menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=770');return false;">
    ...
    <?php if ($this->item->params->get('allow_embedding') != '0') : ?>
    <div class="row-fluid share-panel">
      <div class="span12">
        <label><?php echo JText::_('COM_HWDMS_EMBED'); ?></label>
        <div class="row-fluid share-embed">
          <i class="icon-embed"></i>
          <input type="text" title="<?php echo JText::_('COM_HWDMS_EMBED_CODE'); ?>" value="<?php echo $this->escape($share_embedcode); ?>" name="share_embed" class="share-embed-input span12">
        </div>
      </div>    
    </div>
