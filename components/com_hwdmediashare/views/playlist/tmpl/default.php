...
$layout = $this->params->get('lightbox_mode') != 0 ? 'media_list_lightbox' :  'media_list';
?>
  ...
  <div id="hwd-container" class="<?php echo $this->pageclass_sfx;?>"> <a name="top" id="top"></a>
    ...
    <div class="media-header">
      ...
      <div class="btn-group pull-right">
        ...
        <?php endif; ?>          
        <?php if ($this->playlist->featured && $this->params->get('item_meta_featured_status') != '0') : ?>
          <div class="btn btn-danger btn-noevent"><?php echo JText::_('COM_HWDMS_FEATURED'); ?></div>
        <?php endif; ?>
        <?php if ($this->playlist->status != 1) : ?>
          <div class="btn btn-danger btn-noevent"><?php echo $this->utilities->getReadableStatus($this->playlist); ?></div>
        <?php endif; ?>
        <?php if ($this->playlist->published != 1) : ?>
          <div class="btn btn-danger btn-noevent"><?php echo JText::_('COM_HWDMS_UNPUBLISHED'); ?></div>
        <?php endif; ?> 
        <?php if (count($this->items)): ?>  
          <a title="<?php echo JText::_('COM_HWDMS_PLAY_NOW'); ?>" href="<?php echo JRoute::_(hwdMediaShareHelperRoute::getMediaItemRoute(reset($this->items)->id)); ?>" class="btn media-play-now"><i class="icon-play"></i> <?php echo JText::_('COM_HWDMS_PLAY_NOW'); ?></a>            
        <?php endif; ?>
    ...
    <div class="media-list-view">
      ...
      <?php else : ?>
        <?php echo JLayoutHelper::render($layout, $this, JPATH_ROOT.'/components/com_hwdmediashare/libraries/layouts'); ?>
      <?php endif; ?> 
    </div>           
    <!-- Pagination -->
    ...
    <div class="well media-playlist-description"> 
      ...
      <div class="pull-right">
        <div class="media-info-likes pull-right">      
          <?php if ($this->params->get('item_meta_likes') != '0') :?><i class="icon-thumbs-up"></i> <span id="media-likes"><?php echo (int) $this->playlist->likes; ?></span><?php endif; ?>
          <?php if ($this->params->get('item_meta_dislikes') != '0') :?><i class="icon-thumbs-down"></i> <span id="media-dislikes"><?php echo (int) $this->playlist->dislikes; ?></span><?php endif; ?>
        </div>
      </div>       
      <?php endif; ?>        
      ...
      <?php if ($this->params->get('item_meta_likes') != '0' || $this->params->get('item_meta_dislikes') != '0' || $this->params->get('item_meta_report') != '0') : ?>
        <div class="btn-group">
          <?php if ($this->params->get('item_meta_likes') != '0') : ?>
            <a title="<?php echo JText::_('COM_HWDMS_LIKE'); ?>" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=playlists.like&id=' . $this->playlist->id . '&return=' . $this->return . '&tmpl=component&'.JSession::getFormToken().'=1'); ?>" class="btn btn-mini" id="playlist-like-btn" data-media-task="like" data-media-id="<?php echo $this->playlist->id; ?>" data-media-return="<?php echo $this->return; ?>" data-media-token="<?php echo JSession::getFormToken(); ?>"><i class="icon-thumbs-up"></i> <?php echo JText::_('COM_HWDMS_LIKE'); ?></a>
          <?php endif; ?>
          <?php if ($this->params->get('item_meta_dislikes') != '0') : ?>
            <a title="<?php echo JText::_('COM_HWDMS_DISLIKE'); ?>" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=playlists.dislike&id=' . $this->playlist->id . '&return=' . $this->return . '&tmpl=component&'.JSession::getFormToken().'=1'); ?>" class="btn btn-mini" id="playlist-dislike-btn" data-media-task="dislike" data-media-id="<?php echo $this->playlist->id; ?>" data-media-return="<?php echo $this->return; ?>" data-media-token="<?php echo JSession::getFormToken(); ?>"><i class="icon-thumbs-down"></i> <?php echo JText::_('COM_HWDMS_DISLIKE'); ?></a>
          <?php endif; ?>
          <?php if ($this->params->get('item_meta_report') != '0') : ?>
            <a title="<?php echo JText::_('COM_HWDMS_REPORT'); ?>" href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=playlistform.report&id=' . $this->playlist->id . '&return=' . $this->return . '&tmpl=component'); ?>" class="btn btn-mini media-popup-iframe-form" id="playlist-report-btn"><i class="icon-flag"></i> <?php echo JText::_('COM_HWDMS_REPORT'); ?></a>
          <?php endif; ?>  
        </div>          
      <?php endif; ?>
      <!-- Tags -->
