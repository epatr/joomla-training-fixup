...
class hwdMediaShareViewChannels extends JViewLegacy
{
	...
	{
                // Check if we are trying to view our own channel              
                $app = JFactory::getApplication();
                $user = JFactory::getUser();
                if ($app->input->get('layout', '', 'word') == 'mychannel')
                {
                        if ($user->id)
                        {
                                $app->redirect(hwdMediaShareHelperRoute::getChannelRoute($user->id));
                        }
                        else
                        {
                                $return = base64_encode(hwdMediaShareHelperRoute::getChannelRoute($user->id));
		...
		$this->pageclass_sfx = htmlspecialchars($this->params->get('pageclass_sfx'));
                ...
                $this->display = $this->state->get('media.display_channels', 'details');
	...
	protected function _prepareDocument()
	{
		...
		{
                        $title = JText::_('COM_HWDMS_CHANNELS');
                        $heading = JText::_('COM_HWDMS_CHANNELS');
		}
		...
		{       
                        // Breadcrumb support.
			$path = array(array('title' => JText::_('COM_HWDMS_CHANNELS'), 'link' => ''));
		...
		if ($menu && $menu->query['option'] == 'com_hwdmediashare' && $menu->query['view'] == 'channels' && $this->params->get('menu-meta_description'))
                {
		...
		if ($menu && $menu->query['option'] == 'com_hwdmediashare' && $menu->query['view'] == 'channels' && $this->params->get('menu-meta_keywords'))
                {
