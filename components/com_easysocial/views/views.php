...
class EasySocialSiteView extends EasySocialView
{
	...
	public function __construct($config = array())
	{
		...
		if (!$this->my->hasCommunityAccess()) {
			...
			$allowedViews = array('profile', 'account');
			$allowedLayouts = array('edit', 'requirePasswordReset');
			...
			if (in_array($view, $redirectView) && (!$layout || $layout == 'completed') && !$isLogout && !$isProfileSaving) {
				// we need to redirect the user to profile edit page.
				$this->redirect(ESR::profile(array('layout' => 'edit'), false));
				return;
			}
