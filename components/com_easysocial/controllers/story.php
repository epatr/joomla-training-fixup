...
class EasySocialControllerStory extends EasySocialController
{
	...
	public function checkClusterPermissions($id, $type)
	{
		...
		if ($type == SOCIAL_TYPE_GROUP && !$this->my->canPostClusterStory($type, $id)) {
			return $this->view->exception('COM_EASYSOCIAL_STORY_NOT_ALLOW_TO_POST_IN_GROUP');
		}

		// This is for Page specific stream
		if ($type == SOCIAL_TYPE_PAGE) {

			// Check if the user has permisssion to create a posting
			$page = ES::page($id);
			$permissions = $page->getParams()->get('stream_permissions', null);

			if (!is_null($permissions)) {
				// If the user is not an admin, ensure that permission has follower
				if ($page->isMember() && !in_array('member', $permissions) && !$page->isOwner() && !$page->isAdmin()) {
					return $this->view->exception('COM_EASYSOCIAL_STORY_NOT_ALLOW_TO_POST_IN_PAGE');
				}

				// If the user is an admin, ensure that permission has admin
