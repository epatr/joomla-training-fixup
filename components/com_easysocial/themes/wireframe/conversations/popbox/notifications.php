		...
		<?php if ($conversations) { ?>
			...
			<?php foreach ($conversations as $conversation) { ?>
				<div class="popbox-dropdown-nav__item <?php echo $conversation->isNew() ? 'is-unread' : ''; ?>">
					<?php if (ES::conversekit()->exists() && $conversation->isSingle()) { ?>
						<?php 
						$participants = $conversation->getParticipants();
						$participant = $participants[0];
						?>
						<a href="javascript:void(0);" class="popbox-dropdown-nav__link" data-ck-chat="<?php echo $participant->id;?>">
					<?php } else { ?>
								...
								<div class="popbox-dropdown-nav__post">
									<?php if ($conversation->getLastMessage()) { ?>
										...
										<div class="object-timestamp t-text--muted t-fs--sm">
											<i class="fa fa-clock-o"></i>&nbsp; <?php echo $conversation->getLastRepliedDate()->toLapsed();?>
										</div>
									<?php } ?>
								</div>
							</div>
