...
class EventbookingHelper
{
	...
	public static function generateInvoicePDF($row)
	{
		...
		else
		{
			...
			$itemRate                           = EventbookingHelperRegistration::getRegistrationRate($rowEvent->id, $row->number_registrants);
	...
	public static function downloadCertificates($rows, $config)
	{
		...
		list($fileName, $filePath) = static::generateCertificates($rows, $config);
