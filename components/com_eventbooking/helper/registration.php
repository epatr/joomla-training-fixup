...
class EventbookingHelperRegistration
{
	 ...
	 * @return array
	 */
	public static function getFormFields($eventId = 0, $registrationType = 0, $activeLanguage = null, $userId = null)
	{
	...
	public static function buildTags($row, $form, $event, $config, $loadCss = true)
	{
		...
		else
		{
			...
			if ($event->has_multiple_ticket_types)
			{
				...
				foreach ($registrantTickets as $registrantTicket)
				{
					...
					$replaces['TICKET_TYPE'] = $ticketTypes[$registrantTicket->ticket_type_id]->title;
				}
		...
		$rate                          = static::getRegistrationRate($row->event_id, $row->number_registrants);
	...
	public static function generateTicketNumberQrcode($ticketNumber)
	{
		...
		{
			require_once JPATH_ADMINISTRATOR . '/components/com_eventbooking/libraries/vendor/phpqrcode/qrlib.php';
			QRcode::png($ticketNumber, JPATH_ROOT . '/media/com_eventbooking/qrcodes/' . $filename);
		}
	}
