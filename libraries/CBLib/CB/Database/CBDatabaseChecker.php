...
//			. "pf.name AS fieldpluginname, pf.published AS fieldpluginpublished "
	...
	public static function renderDatabaseResults( &$dbChecker, $upgrade, $dryRun, $result, $messagesBefore, $messagesAfter, $dbName, $dbId, $showConclusion = true ) {
		...
		if ( $messagesBefore ) {
			$return					.=		'<div class="form-group row no-gutters cb_form_line cbDbResultsMsgs">';
		...
		if ( $dbChecker !== null ) {
			$return					.=		'<div class="form-group row no-gutters cb_form_line cbDbResultsCheck">';
			...
			if ( $result == true ) {
				...
				if ( ! $upgrade ) {
					$return			.=			'<div class="text-danger cbDbResultsErrorsFix">'
									...
									.				' <span class="alert alert-danger text-large">'
		...
		if ( $showConclusion ) {
			if ( $upgrade ) {
				if ( $dryRun ) {
					$return			.=		'<div class="form-group row no-gutters cb_form_line cbDbResultsConclusion">'
					...
					$return			.=		'<div class="form-group row no-gutters cb_form_line cbDbResultsConclusion">'
									...
									.			' <span class="alert alert-warning text-large">'
