...
class cbFieldHandler extends cbPluginHandler
{
	...
	protected function renderFieldHtml( $field, $user, $oValue, $output, $formatting, $reason, $rowClasses )
	{
		...
		$labelFor							=	( ( ( $output == 'htmledit' ) && $field->name ) ? htmlspecialchars( moscomprofilerHTML::htmlId( $field->name . ( preg_match( '/multicheckbox|multiselect/', $field->type ) ? '[]' : null ) ) ) : 'cbfv_' . $field->fieldid );

		if ( $field->cssclass ) {
			$rowClasses[]					=	$field->cssclass;
		}

		if ( preg_match( '/^(?:<(select|input|textarea|button)|(?:<[^<]+(?:form|btn)-(group)[^>]+>))/i', trim( $oValue ), $matches ) ) {
		...
		switch ( $formatting ) {
			...
			case 'div':
				...
				$results					.=		'<div class="cb_field ' . $colClass . '">'
											.			'<div id="cbfv_' . $field->fieldid . '"' . ( ( ! $tag ) && ( ! $twoLines ) ? ' class="form-control-plaintext"' : null ) . '>'
											.				$oValue
											.			'</div>'
											.		'</div>'
											.	'</div>';
	...
	protected function _fieldEditToHtml( &$field, &$user, $reason, $tag, $type, $value, $additional, $allValues = null, $displayFieldIcons = true, $classes = null )
	{
		...
		switch ( $type ) {
			...
			case 'radiobuttons':
				...
				$htmlInput		=	moscomprofilerHTML::radioListButtons( $allValues, $inputName, $tooltip, 'value', 'text', $value, $oReq, $classes, $additional );
				break;
			...
			case 'multicheckboxbuttons':
				$tooltip		=	( $trimmedDescription && $inputDescription ? cbTooltip( $_CB_framework->getUi(), $htmlDescription, $translatedTitle, null, null, null, null, 'data-hascbtooltip="true"' ) : '' );
				$htmlInput		=	moscomprofilerHTML::checkboxListButtons( $allValues, $inputName . '[]', $tooltip, 'value', 'text', $this->_explodeCBvaluesToObj( $value ), $oReq, $classes, $additional );
				break;

			case 'checkbox':
				$classes[]		=	'form-check-input mr-0';

				if ( $classes ) {
					$additional	.=	' class="' . implode( ' ', $classes ) . '"';
				}
				$tooltip		=	( $trimmedDescription && $inputDescription ? cbTooltip( $_CB_framework->getUi(), $htmlDescription, $translatedTitle, null, null, null, null, $additional ) : $additional );
				$htmlInput		=	'<div class="cbSingleCntrl form-check form-check-inline">'
								.		'<input type="checkbox" id="' . htmlspecialchars( moscomprofilerHTML::htmlId( $inputName ) ) . '" name="' . htmlspecialchars( $inputName ) . '" value="' . htmlspecialchars( $value ) . '"' . $tooltip . ' />'
								.	'</div>';
				break;
			...
			case 'yesno':
				...
				$additional		.=	( $trimmedDescription && $inputDescription ? cbTooltip( $_CB_framework->getUi(), $htmlDescription, $translatedTitle, null, null, null, null, 'data-hascbtooltip="true"' ) : '' );
				$htmlInput		=	moscomprofilerHTML::yesnoButtonList( $inputName, $additional, $value );
				break;
