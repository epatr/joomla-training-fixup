			...
			if ( $oMenu[$pos] || $oMenuNest[$pos] ) {
				...
				if ( ! $oMenuJS++ ) {
					$js			=	"$( '.cbTabsMenuNavBar' ).on( 'click', '.cbTabsMenuNavBarToggle', function() {"
								...
								.			"if ( nested.length ) {"
								.				"cbtabs.tabsNav.siblings( '.cbTabNavMore' ).find( '.cbSubMenu' ).append( nested.clone( true ) ).find( '.nav-item' ).removeClass( 'nav-item' ).find( '.nav-link' ).removeClass( 'nav-link' ).addClass( 'dropdown-item' );"
				...
				if ( $oMenuNest[$pos] ) {
					$more		=	'<div class="cbTabNavMore cbTabNavMenuMore cbTooltip nav-item dropdown mb-auto" data-cbtooltip-tooltip-target="#cbtabs' . htmlspecialchars( 'CBMenu' . $pos ) . 'More" data-cbtooltip-menu="true" data-cbtooltip-classes="qtip-nostyle cbTabNavMoreDropdown" data-cbtooltip-open-classes="show">'
								.		'<button type="button" class="cbTabNavMenuMoreBtn nav-link navbar-toggler d-none d-sm-block">'
								.			'<span class="navbar-toggler-icon"></span>'
								.		'</button>'
								.		'<ul id="cbtabs' . htmlspecialchars( 'CBMenu' . $pos ) . 'More" class="cbSubMenu list-unstyled dropdown-menu"></ul>'
								.	'</div>';
				}
	...
	public function getEditTabs( &$user, $postdata = null, $output = 'htmledit', $formatting = 'table', $reason = 'edit', $tabbed = true )
	{
		...
		foreach( $oTabs AS $oTab ) {
			...
			if ( $tabbed && ( $tabContent != '' ) ) {
				...
				$results					.=	$this->startTab( 'CB', $tabTitle, $oTab->tabid, array( 'tab' => $oTab->cssclass, 'link' => ( (int) $tabbed === 2 ? 'disabled' : null ), 'pane' => $oTab->cssclass ) )
											.		'<div class="cb_tab_content cb_tab_main" id="cb_tabid_' . $oTab->tabid . '">'
											.			$tabContent
											.		'</div>';

				if ( (int) $tabbed === 2 ) {
					$results				.=		'<div class="row no-gutters cb_tab_content_nav cbTabsStepByStepNav">'
		...
		if ( $tabbed ) {
			if ( (int) $tabbed === 2 ) {
				$return						=	$this->startPane( 'CB', array( 'container' => 'cbTabsStepByStep', 'nav' => 'nav nav-pills mb-3', 'override' => true ), array( 'container' => 'data-cbtabs-step-by-step="true"' ) );
			} else {
