	...
	public function displayWeb()
	{
		...
		foreach ( $packages as $typeId => $type ) {
			...
			foreach ( $type->subTree( 'packages' ) as $packageId => $typePackage ) {
				...
				if ( ( ! $typePackage->get( 'conditioned', false, GetterInterface::BOOLEAN ) ) && ( $isLatest === -1 ) ) {
			...
			$menu										.=	'<li class="cbWebType' . ( ! count( $items[$typeValue] ) ? ' cbWebTypeEmpty' : null ) . ' nav-item">'
														.		'<a href="javascript: void(0);" class="cbWebTypeLink nav-link rounded-0 m-0 clearfix" data-cbweb-type="' . htmlspecialchars( $typeValue ) . '">'
														.			'<span class="fa fa-' . htmlspecialchars( $icon ) . '" style="width: 20px;"></span> ' . $type->get( 'label', null, GetterInterface::HTML )
														.			'<span class="badge badge-pill badge-primary pull-right" style="font-size: 75%;">' . $type->get( 'count', 0, GetterInterface::INT ) . '</span>'
														.		'</a>'
			...
			foreach ( $type->subTree( 'subtypes' ) as $subTypeId => $subType ) {
				...
				$menu									.=			'<li class="cbWebSubType nav-item">'
														.				'<a href="javascript: void(0);" class="cbWebSubTypeLink nav-link rounded-0 m-0 clearfix" data-cbweb-type="' . htmlspecialchars( $subTypeValue ) . '">'
														...
														.					'<span class="badge badge-pill badge-primary pull-right" style="font-size: 75%;">' . $subType->get( 'count', 0, GetterInterface::INT ) . '</span>'
														.				'</a>'
														.			'</li>';
				...
				foreach ( $subType->subTree( 'packages' ) as $packageId => $subTypePackage ) {
					...
					if ( ( ! $subTypePackage->get( 'conditioned', false, GetterInterface::BOOLEAN ) ) && ( $isLatest === -1 ) ) {
						if ( ! in_array( $packageId, $updates ) ) {
							$updates[]					=	$packageId;
						}
		...
		$return											=	( ! $request ? '<div class="cbWeb">' : null )
														.	'<div class="cbWebResponse row no-gutters">'
														.		'<div class="cbWebMenu col-md-3 pr-md-3 mb-3">'
														.			'<ul class="cbWebTypes nav flex-column nav-pills bg-light">';

		if ( $news || $items['home'] ) {
			$return										.=				'<li class="cbWebType nav-item">'
														.					'<a href="javascript: void(0);" class="cbWebTypeLink nav-link rounded-0 m-0' . ( ! $items['updates'] ? ' active' : null ) . '" data-cbweb-type="home">'
														.						'<span class="fa fa-home" style="width: 20px;"></span> ' . CBTxt::T( 'Home' )
														.						( $items['home'] ? '<span class="badge badge-pill badge-primary pull-right" style="font-size: 75%;">' . count( $items['home'] ) . '</span>' : null )
														.					'</a>'
														.				'</li>';
		}
		...
		if ( $items['updates'] ) {
			$return										.=				'<li class="cbWebType nav-item">'
														.					'<a href="javascript: void(0);" class="cbWebTypeLink nav-link rounded-0 m-0 active" data-cbweb-type="updates">'
														.						'<span class="fa fa-undo" style="width: 20px;"></span> ' . CBTxt::T( 'Updates' )
														.						'<span class="badge badge-pill badge-primary pull-right" style="font-size: 75%;">' . count( $items['updates'] ) . '</span>'
														.					'</a>'
														.				'</li>';
		}
		...
		$return											.=				$menu
														.			'</ul>'
														.		'</div>'
														.		'<div class="cbWebPackages col-md-9">';
		...
		foreach ( $items as $type => $typeItems ) {
			...
			$return										.=			'<div class="' . ( $type !== 'updates' ? 'row no-gutters ' : null ) . 'cbWebPackagesType' . ( $type === 'home' ? ( $items['updates'] ? ' hidden' : null ) : ( $type !== 'updates' ? ' hidden' : null ) ) . '" data-cbweb-type="' . htmlspecialchars( $type ) . '">';
			...
			if ( $type == 'updates' ) {
				$return									.=				'<div class="text-right mb-2 cbWebUpdateAll">'
														...
														.				'</div>'
														.				'<div class="row no-gutters cbWebUpdates">';
			}

			foreach ( $typeItems as $itemId => $item ) {
				/** @var ParamsInterface $item */
				$return									.=					'<div class="col-md-6 pb-2' . ( $i % 2 ? ' pl-md-1' : ' pr-md-1' ) . ' cbWebPackage">'
														.						'<div class="card h-100 cbWebPackageInner">'
														.							'<div class="card-header p-2 cbWebPackageHeader">'
														.								'<div class="row no-gutters">'
														.									'<div class="col-12 col-sm">'
														.										'<div class="d-inline-block text-large">'
														...
														.									'</div>'
														.									'<div class="col-12 col-sm flex-grow-0 mt-1 mt-sm-0 pl-sm-2 text-nowrap text-right">';

				if ( $item->get( 'demo', null, GetterInterface::STRING ) ) {
					$return								.=										'<a href="' . htmlspecialchars( $item->get( 'demo', null, GetterInterface::STRING ) ) . '" target="_blank" rel="nofollow" class="btn btn-sm btn-sm-block btn-info">' . CBTxt::T( 'Demo' ) . '</a> ';
				}

				if ( ! $item->get( 'conditioned', false, GetterInterface::BOOLEAN ) ) {
					if ( $item->get( 'type', null, GetterInterface::INT ) == 12 ) {
						$return							.=										'<a href="' . htmlspecialchars( $item->get( 'download', null, GetterInterface::STRING ) ) . '" target="_blank" rel="nofollow" class="btn btn-sm btn-sm-block btn-primary">' . CBTxt::T( 'Download' ) . '</a>';
					} else {
						if ( $item->get( 'latest', 0, GetterInterface::INT ) === 1 ) {
							$return						.=										'<a href="javascript: void(0);" data-cbweb-file="' . htmlspecialchars( $item->get( 'name', null, GetterInterface::STRING ) ) . '" data-cbweb-download="' . htmlspecialchars( $item->get( 'download', null, GetterInterface::STRING ) ) . '" class="btn btn-sm btn-sm-block btn-secondary cbWebPackageDownload">' . CBTxt::T( 'Already Installed' ) . '</a>';
						} elseif ( $item->get( 'latest', 0, GetterInterface::INT ) === -1 ) {
							$return						.=										'<a href="javascript: void(0);" data-cbweb-file="' . htmlspecialchars( $item->get( 'name', null, GetterInterface::STRING ) ) . '" data-cbweb-download="' . htmlspecialchars( $item->get( 'download', null, GetterInterface::STRING ) ) . '" class="btn btn-sm btn-sm-block btn-danger cbWebPackageDownload">' . CBTxt::T( 'Download & Update' ) . '</a>';
						} else {
							$return						.=										'<a href="javascript: void(0);" data-cbweb-file="' . htmlspecialchars( $item->get( 'name', null, GetterInterface::STRING ) ) . '" data-cbweb-download="' . htmlspecialchars( $item->get( 'download', null, GetterInterface::STRING ) ) . '" class="btn btn-sm btn-sm-block btn-primary cbWebPackageDownload">' . CBTxt::T( 'Download & Install' ) . '</a>';
						}
					}
				} else {
					...
					$return								.=										'<a href="' . htmlspecialchars( $item->get( 'download', null, GetterInterface::STRING ) ) . '" target="_blank" rel="nofollow" class="btn btn-sm btn-sm-block btn-primary">' . $button . '</a>';
				}

				$return									.=									'</div>'
														.								'</div>'
														.							'</div>'
														.							( $item->get( 'description', null, GetterInterface::HTML ) ? '<div class="card-body p-2 cbWebPackageDescription">' . $item->get( 'description', null, GetterInterface::HTML ) . '</div>' : null )
														.							'<div class="card-footer p-0 cbWebPackageDetails">'
														.								'<div class="ml-2 mr-2 mt-2 mt-sm-1 mb-2 cbWebPackageDetailsInner">';

				if ( $item->get( 'version', null, GetterInterface::STRING ) ) {
					$return								.=									'<span class="d-block d-sm-inline-block w-sm-100 mt-1 align-bottom cbWebPackageDetailsVersion">'
														.										'<span class="d-inline-block w-sm-100 badge badge-light border p-2 font-weight-normal text-wrapall cbWebPackageVersion">' . CBTxt::T( 'WEB_PACKAGE_VERSION', 'Version: [version]', array( '[version]' => htmlspecialchars( $item->get( 'version', null, GetterInterface::STRING ) ) ) ) . '</span>'
														.									'</span> ';
				}

				if ( $item->get( 'size', null, GetterInterface::STRING ) ) {
					$return								.=									'<span class="d-block d-sm-inline-block w-sm-100 mt-1 align-bottom cbWebPackageDetailsSize">'
														.										'<span class="d-inline-block w-sm-100 badge badge-light border p-2 font-weight-normal text-wrapall cbWebPackageSize">' . CBTxt::T( 'WEB_PACKAGE_SIZE', 'Size: [size]', array( '[size]' => htmlspecialchars( $item->get( 'size', null, GetterInterface::STRING ) ) ) ) . '</span>'
														.									'</span> ';
				}

				if ( $item->get( 'date', null, GetterInterface::STRING ) ) {
					$return								.=									'<span class="d-block d-sm-inline-block w-sm-100 mt-1 align-bottom cbWebPackageDetailsDate">'
														.										'<span class="d-inline-block w-sm-100 badge badge-light border p-2 font-weight-normal text-wrapall cbWebPackageDate">' . CBTxt::T( 'WEB_PACKAGE_DATE', 'Date: [date]', array( '[date]' => htmlspecialchars( $item->get( 'date', null, GetterInterface::STRING ) ) ) ) . '</span>'
														.									'</span> ';
				}

				if ( $item->get( 'hits', null, GetterInterface::STRING ) ) {
					$return								.=									'<span class="d-block d-sm-inline-block w-sm-100 mt-1 align-bottom cbWebPackageDetailsHits">'
														.										'<span class="d-inline-block w-sm-100 badge badge-light border p-2 font-weight-normal text-wrapall cbWebPackageHits">' . CBTxt::T( 'WEB_PACKAGE_HITS', 'Hits: [hits]', array( '[hits]' => htmlspecialchars( $item->get( 'hits', null, GetterInterface::STRING ) ) ) ) . '</span>'
														.									'</span> ';
				}
		...
		if ( $cacheTime ) {
			$return										.=	'<div class="text-right text-muted cbWebCached">'
