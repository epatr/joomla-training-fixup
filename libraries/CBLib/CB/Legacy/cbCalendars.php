...
class cbCalendars
{
	...
	public function __construct( $ui, $calendarType = null, $dateFormat = null, $timeFormat = null )
	{
		...
		$this->defDateFormat			=	( $dateFormat !== null ? $dateFormat : $ueConfig['date_format'] );
	...
	public function cbAddCalendar( $name, $label = null, $required = false, $value = null, $readOnly = false, $showTime = false, $minYear = null, $maxYear = null, $attributes = null, $serverTimeOffset = true, $offsetOverride = null )
	{
		...
		if ( $readOnly ) {
			...
			if ( ( $showTime !== 2 ) && ( $minYear !== null ) && ( $maxYear !== null ) ) {
			...
			if ( in_array( $this->calendarType, array( 1, 4 ) ) || $addPopup ) {
				...
				if ( $showTime ) {
					...
					$attributes				.=	' data-cbdatepicker-showtime="true" data-cbdatepicker-timeformat="' . htmlspecialchars( $this->timeFormat[1][2] ) . '" data-cbdatepicker-timetemplate="' . htmlspecialchars( $this->timeFormat[1][1] ) . '"';
				}
			}
