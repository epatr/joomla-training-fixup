...
class RegistryEditView {
	 ...
	 */
	protected function jsQuote( $text ) {
	 ...
	 */
	protected function jsCleanQuote( $fieldValue, $type ) {
	...
	protected function _renderLine( $param, $result, $control_name='params', $htmlFormatting = 'div', $htmlid = true, /** @noinspection PhpUnusedParameterInspection */ $view = false ) {
		...
		if ( ! in_array( $htmlFormatting, array( 'none', 'fieldsListArray' ) ) ) {
			if ( $type == 'yesno' ) {
				...
				$tag		=	'group';
			} elseif ( preg_match( '/^(?:<(select|input|textarea|button)|(?:<[^<]+form-(group)[^>]+>))/i', trim( $result[1] ), $matches ) ) {
		...
		if ( $htmlFormatting == 'table' ) {
			...
			$html[]			=	'<div' . $htid . ' class="cbft_' . $type . ( $tag ? ' cbtt_' . $tag : null ) . ' form-group row no-gutters cb_form_line' . ( $twoLine ? ' cbtwolinesfield' : null ) . ( $classes ? ' ' . htmlspecialchars( $classes ) : null ) . '">';
	...
	*/
		...
		switch ( $param->getName() ) {
					...
					if ( $repeat ) {
						...
						foreach ( $valueArray as $index => $value ) {
							...
							if ( in_array( $htmlFormatting, array( 'div', 'span', 'none' ) ) ) {
								$return		.=		'<div class="cbRepeatRow row no-gutters border rounded mb-2 p-2 mw-100">';
								...
								$return		.=			'<div class="cbRepeatRowParams col-12 col-sm">'
								...
								$return		.=		'<table class="table table-bordered m-0 cbRepeatRow">'
											.			'<tbody>'
											.				'<tr>';
						...
						if ( $return ) {
							...
							if ( $viewType != 'view' ) {
								$return		=	'<div class="cbRepeat mw-100" data-cbrepeat-fallback-for="' . htmlspecialchars( $this->control_id( null, $parent_cname ) ) . '" data-cbrepeat-fallback-name="' . htmlspecialchars( $parent_cname ) . '"' . ( ! $repeatOrdering ? ' data-cbrepeat-sortable="false"' : null ) . ( $repeatIgnore ? ' data-cbrepeat-ignore="' . htmlspecialchars( $repeatIgnore ) . '"' : null ) . ( $repeatMax ? ' data-cbrepeat-max="' . (int) $repeatMax . '"' : null ) . '>'
											.		$return
											.		'<div class="cbRepeatRowIncrement form-inline flex-row-reverse">';
			...
			case 'gridcol':
				if ( $htmlFormatting != 'fieldsListArray' ) {
					...
					$order					=	explode( ',', $param->attributes( 'order' ) );

					if ( ! $order ) {
						$order				=	explode( ',', $param->attributes( 'push' ) );
					}

					if ( ! $order ) {
						$order				=	explode( ',', $param->attributes( 'pull' ) );
					}
	 ...
	 */
	protected function _list_options( $name, &$node, $control_name, &$options, $children, $otgroups = true, $value = null, $ignoreClass = false, &$index = 0 )
	{
		$this->_list_options_main( $name, $node, $control_name, $options, $children, $otgroups, $value, $ignoreClass, $index );
	 ...
	 * @param  int                 $index         The option index
	 */
	protected function _list_options_main( $name, &$node, $control_name, &$options, $children, $otgroups = true, $value = null, $ignoreClass = false, &$index = 0 ) {
		$translate												=	$node->attributes( 'translate' );
		...
		if ( $children ) foreach ( $children as $option ) {
			...
			if ( $option->getName() == 'option' ) {
				...
				$this->_list_options_main( $name, $node, $control_name, $options, $option->children(), $otgroups, $value, $ignoreClass, $index );
				...
				if ( $option->attributes( 'type' ) == 'showhide' ) {
					...
					$this->_list_options_main( $name, $node, $control_name, $ifOptions, $option->children(), $otgroups, $value, $ignoreClass, $index );
					...
					if ( $insideParamToRender ) {
						$this->_list_options_main( $name, $node, $control_name, $options, $insideParamToRender->children(), $otgroups, $value, $ignoreClass, $index );
					}
				}
			}
		}
