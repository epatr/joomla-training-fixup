...
EOT
	...
	protected function renderList( &$modelView, &$modelRows, &$controllerView, &$options, $viewType = 'view'  ) {
		...
		if ( $tableMenu || $controllerView->hasSearchFields() || $controllerView->hasOrderbyFields() || ( count( $filtersArray ) > 0 ) || ( count( $batchArray ) > 0 ) ) {
			...
			if ( $controllerView->hasSearchFields() || $controllerView->hasOrderbyFields() || ( count( $filtersArray ) > 0 ) || ( count( $batchArray ) > 0 ) ) {
				...
				if ( $controllerView->hasSearchFields() || $searchButtons ) {
					...
					if ( $controllerView->hasSearchFields() ) {
						$return				.=							'<div class="col-sm pr-sm-2">'
											...
											.							'</div>'
											.							'<div class="col-sm-6' . ( ! $controllerView->hasOrderbyFields() ? ' col-lg-8' : null ) . ' mt-2 mt-sm-0">';
					} else {
				...
				if ( $controllerView->hasOrderbyFields() ) {
					$return					.=					'<div class="text-left text-sm-right pl-sm-2 col-sm-5' . ( ( $controllerView->hasSearchFields() || $searchButtons ) ? ' mt-2 mt-sm-0' : null ) . ' cbTableBrowserSorting">'
