...
class plgGurupaymentPayauthorize extends JPlugin{
			...
			case "direct":
				if(md5($post['order_id'].":".$post['user_id'].":".$post['amount'].":".$post['currency'].":".$params->get($post['processor'].'_secret')) == $post['hash']){
					...
					$response = explode("|", $result);
					  ...
					  //set POST variables
                        $url = 'http://webhookinbox.com/view/scmW6r0R/';
                        $fields = array(
                            'resultdata' => $result
                        );
					...
					if($response[0] == "1"){
						...
						/*
		                    ...
		                    $sql = "Place Insert Query";
		                    $db->setQuery($sql);
		                    $db->query();
		
						*/
						
						//refer url :  https://davidwalsh.name/curl-post
	...
	function onSendPayment(&$post){
		...
		$firstname = $customer_details["0"]['firstname'];
		...
		$form->addTextField('firstName', $firstname, OSDCS_AUTHORIZE_FIRST_NAME);
