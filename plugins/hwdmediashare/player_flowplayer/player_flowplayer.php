...
class plgHwdmediasharePlayer_FlowPlayer extends JObject
{
	...
	public function preloadAssets($params)
	{
                ...
                if (!defined('COM_HWDMS_FLOW_ASSETS_JS'))
                {
                        if ($params->get('hosting') == 'cloud')
                        {
                                $doc->addScript("//releases.flowplayer.org/6.0.5/commercial/flowplayer.min.js");
                        }
                        else
                        {
                        ...
                        // Define the checking constant
                        define('COM_HWDMS_FLOW_ASSETS_JS', true);
                }
                ...
                if ($params->get('asf'))
                {
                        if (!defined('COM_HWDMS_FLOW_ASSETS_ASF'))
                        {
                                $doc->addScript("//s0.2mdn.net/instream/html5/ima3.js");
                                $doc->addScript($params->get('asfjs', '//releases.flowplayer.org/asf/flowplayer.org/1.1.1/asf.min.js'));
                                //$doc->addScript($params->get('asfjs'));
                                $doc->addStyleSheet("//releases.flowplayer.org/asf/flowplayer.org/1.1.1/asf.min.css");
                                ...
                                // $doc->addStyleDeclaration(".flowplayer.ad-is-visible .fp-ui .ad-time { visibility: hidden !important; }");

                                // Initialize the plugin (this needs to be in the player container)
...
//                                ");
                                ...
                                // Define the checking constant
                                define('COM_HWDMS_FLOW_ASSETS_ASF', true);
                        }
                }           
        }
...
<div class="media-respond" style="max-width:<?php echo $config->get('mediaitem_size', '500'); ?>px;">
  ...
  <div id="media-flow-<?php echo $item->id; ?>" class="media-content">
    ...
    >
      ...
      <?php if (file_exists(JPATH_ROOT . '/images/webvtt/captions-' . $item->id. '.vtt')) : ?><track kind="subtitles" src="<?php echo JURI::root( true ); ?>/images/webvtt/captions-<?php echo $item->id; ?>.vtt" srclang="en" /><?php endif; ?>
      <?php if (file_exists(JPATH_ROOT . '/images/webvtt/chapters-' . $item->id. '.vtt')) : ?><track kind="chapters" src="<?php echo JURI::root( true ); ?>/images/webvtt/chapters-<?php echo $item->id; ?>.vtt" srclang="en" /><?php endif; ?>     
    </video>
  </div>
</div>
