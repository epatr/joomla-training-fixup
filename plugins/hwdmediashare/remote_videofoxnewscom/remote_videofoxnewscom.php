...
/**
 * @package     Joomla.site
 * @subpackage  Plugin.hwdmediashare.remote_videofoxnewscom
 *
...
class plgHwdmediashareRemote_videofoxnewscom extends hwdMediaShareRemote
{    
	/**
	...
	/**
	 * Returns the plgHwdmediashareRemote_videofoxnewscom object, only creating it if it
	 * doesn't already exist.
	 *
	 * @access  public
	 * @return  object  The plgHwdmediashareRemote_videofoxnewscom object.
	 */
	public static function getInstance()
	{
		...
		if (!isset ($instance))
                {
			$c = 'plgHwdmediashareRemote_videofoxnewscom';
                        $instance = new $c;
		}
	...
	public function display($item)
	{
		// Initialise variables.
                ...
                // Load plugin.
		$plugin = JPluginHelper::getPlugin('hwdmediashare', 'remote_videofoxnewscom');
		
                // Load the language file.
                $lang = JFactory::getLanguage();
                $lang->load('plg_hwdmediashare_remote_videofoxnewscom', JPATH_ADMINISTRATOR, $lang->getTag());

                if (!$plugin)
                {
                        $this->setError(JText::_('PLG_HWDMEDIASHARE_REMOTE_VIDEOFOXNEWSCOM_ERROR_NOT_PUBLISHED'));
                        return false;
                }
                ...
                {
                        $this->setError(JText::_('PLG_HWDMEDIASHARE_REMOTE_VIDEOFOXNEWSCOM_ERROR_PLAYBACK_PROBLEM_SEE_ORIGINAL'));
                        return false;
                }
	}
	...
	public function getDirectDisplayLocation($item)
	{
		...
		$plugin = JPluginHelper::getPlugin('hwdmediashare', 'remote_videofoxnewscom');
                ...
                {             
                        ...
                        return JURI::getInstance()->getScheme() .'://' . parse_url($embedLookup, PHP_URL_HOST) . parse_url($embedLookup, PHP_URL_PATH) . '?' . parse_url($embedLookup, PHP_URL_QUERY);
                }
        }  
	...
	{
                $buffer = $this->getBuffer($item->source);
                if (!empty($buffer))
                {
                        preg_match('/<meta name="twitter:player" content="([^"]+)/', $buffer, $match);
                        if (!empty($match[1]))
                        {
                                return $match[1];
                        }
