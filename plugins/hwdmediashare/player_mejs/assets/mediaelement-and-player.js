			...
			if (t.height.toString().indexOf('%') > 0 || (t.$node.css('max-width') !== 'none' && t.$node.css('max-width') !== 't.width') || (t.$node[0].currentStyle && t.$node[0].currentStyle.maxWidth === '100%')) {
				...
				t.layers.children('.mejs-layer')
					...
					.height(t.height);
                        ...
                        // START HWD 2014-09-24 Modify the aspect padding to respond to any height changes.
                        var HWDaspect = parseInt(nativeHeight/nativeWidth * 100);
                        var HWDheight = parseInt(parentWidth * nativeHeight/nativeWidth);
	
                        var HWDid = parseInt(t.node.id.replace('media-mejs-', '')); 
                        $('#media-aspect-' + HWDid).css('padding-top', HWDheight + 'px'); 
                        this.container.css('height', HWDheight + 'px'); 
                        // END HWD 2014-09-24
		},
