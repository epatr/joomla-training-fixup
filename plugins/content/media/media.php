...
class plgContentMedia extends JPlugin
{
			...
			foreach ($matches as $match)
                        {
                                ...
                                {
                                        // Sanitise data passed in content code.
                                        list($parameter, $value) = explode('=', $list);
                                ...
                                // Set the mediaitem_size option using the width option passed in the content code.
                                $this->config->set('mediaitem_size', $this->config->get('width', 1080));
	...
	{         
		...
		if ($model->getError())
                {
                        return $utilities->printNotice($model->getError(), '<p><a href="' . hwdMediaShareHelperRoute::getMediaItemRoute($this->config->get('id')) . '" class="btn">' . JText::_('PLG_CONTENT_MEDIA_BUTTON_VIEW_MEDIA') . '</a></p>');         
		}
                elseif ($item === false)
                {
                        return '<!-- Failed media plugin: ' . $this->config . '-->';
	...
	{         
		// Initialise variables.
                ...
                // Populate the model states.
                $model->populateState();
                $model->setState('filter.album_id', $this->config->get('id'));
         
                // Load the album and items.
		...
		if ($model->getError())
                {
                        return $utilities->printNotice($model->getError(), '<p><a href="' . hwdMediaShareHelperRoute::getAlbumRoute($this->config->get('id')) . '" class="btn">' . JText::_('PLG_CONTENT_MEDIA_BUTTON_VIEW_MEDIA') . '</a></p>');         
		}
                elseif ($album === false)
                {
                        return '<!-- Failed media plugin: ' . $this->config . '-->';
                ...
                $helper->album = $album;
                $helper->items = $items;
                $helper->columns = $this->config->get('columns', 3);
                $helper->display = $this->config->get('layout', 'details');
                $helper->utilities = hwdMediaShareUtilities::getInstance();
                $helper->return = base64_encode(JFactory::getURI()->toString());

                // Setup association
                $album_id_existing = $app->input->get('album_id');
                $app->input->set('album_id', $helper->album->id);
                
                $layout = JPATH_ROOT.'/components/com_hwdmediashare/libraries/layouts/mediaitem_layout_album.php';
                if (JFile::exists($layout))
                {
                        $return = JLayoutHelper::render('mediaitem_layout_album', $helper, JPATH_ROOT.'/components/com_hwdmediashare/libraries/layouts');
                }
                else
                {
                        $return = '<!-- Failed media plugin: ' . $this->config . '-->';
                ...
                // Reset association
                $app->input->set('album_id', $album_id_existing);    
