...
function generateTreeCategoriesList($parent_id, $level){
    ...
    $sql = "select c.id, c.name, c.groups from #__guru_category c, #__guru_categoryrel cr where c.id=cr.child_id and cr.parent_id=".intval($parent_id)." and c.published=1";
    $db->setQuery($sql);
    $db->execute();
