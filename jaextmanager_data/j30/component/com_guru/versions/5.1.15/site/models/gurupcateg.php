...
class guruModelguruPcateg extends JModelList {
	...
	function getlistPcategs(){ 
		if(empty($this->_promos)){
			$query = "SELECT c.*, count( parent_id ) AS copii FROM #__guru_category c LEFT OUTER JOIN #__guru_categoryrel r ON c.id = r.child_id WHERE c.id IN (select child_id from #__guru_categoryrel) and r.parent_id = 0 and c.published=1 GROUP BY c.id, c.name, c.alias, c.published, c.description, c.image, c.ordering, c.icon, c.groups order by c.ordering asc";
	...
	function getchildren(){
		...
		$query  = "SELECT c.*, count(catid) as copii FROM #__guru_category c LEFT JOIN #__guru_program t ON t.catid=c.id WHERE c.id in (select child_id from #__guru_categoryrel where parent_id=".intval($idu).") and c.published=1 GROUP BY c.id, c.name, c.alias, c.published, c.description, c.image, c.ordering, c.icon order by c.ordering asc";
		$this->_promos = $this->_getList($query);
		return $this->_promos;
	}
