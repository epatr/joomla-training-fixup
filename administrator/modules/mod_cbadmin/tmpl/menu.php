...
if ( Application::MyUser()->isAuthorizedToPerformActionOnAsset( 'core.manage', 'com_comprofiler' ) ) {
	...
	if ( $params->get( 'menu_plugins', 1 ) ) {
		...
		$_PLUGINS->trigger( 'mod_onCBAdminMenu', array( &$pluginMenu, $disabled, $params->get( 'menu_compact', 0 ) ) );
	}
	...
	if ( $params->get( 'menu_cb', 1 ) && file_exists( $_CB_framework->getCfg( 'absolute_path' ) . '/components/com_comprofiler' ) ) {
		...
		$pluginSubMenu					=	array( array( 'title' => CBTxt::Th( 'Install New Plugin' ), 'link' => $_CB_framework->backendViewUrl( 'installcbplugin', true, array( 'cbprevstate' => base64_encode( $prevStateBase . '&view=showPlugins' ) ) ), 'access' => array( 'core.admin', 'root' ), 'icon' => 'cb-upload' ) );

		if ( $params->get( 'menu_compact', 0 ) && $pluginMenu ) {
			$pluginSubMenu[]			=	array( 'title' => 'spacer' );

			$pluginSubMenu				=	( $pluginSubMenu + $pluginMenu );
		}
		...
		$menu['cb']						=	$cbMenu;
	} elseif ( $params->get( 'menu_compact', 0 ) && $pluginMenu ) {
		$cbMenu							=	array();
		$cbMenu['component']			=	array(	'title' => CBTxt::Th( 'Community Builder' ) );
