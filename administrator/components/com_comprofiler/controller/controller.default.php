...
$task				=	Application::Input()->get( 'view', Application::Input()->get( 'task' ) );
...
function loadSampleData() {
	...
	if ( ! $tab->tabid ) {
		...
		if ( $tab->getError() || ( ! $tab->store() ) ) {
			$return		.=	'<div class="form-group row no-gutters cb_form_line text-danger">'
	...
	if ( $affected ) {
		$return			.=	'<div class="form-group row no-gutters cb_form_line text-success">'
...
function syncUsers() {
	...
	foreach ( $messages as $msg ) {
		if ( $msg ) {
			echo '<div class="form-group row no-gutters cb_form_line">' . $msg . '</div>';
		}
	}
	...
	try {
		...
		print('<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('SQL error %s'), $e->getMessage() ) . '</div>');
		return;
	}

	$affected		=	$_CB_database->getAffectedRows();
	if ($affected) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf(CBTxt::T('Deleted %s not allowed user id 0 entry.'), $affected) . '</div>';
	}
	...
	$affected		=	$_CB_database->getAffectedRows();
	if ($affected) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf(CBTxt::T('Added %s new entries to Community Builder from users Table.'), $affected) . '</div>';
	}
	...
	$affected		=	$_CB_database->getAffectedRows();
	if ($affected) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf(CBTxt::T('Fixed %s existing entries in Community Builder: fixed wrong user_id.'), $affected) . '</div>';
	}
	...
	if (count($users)) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf(CBTxt::T('Removing %s entries from Community Builder missing in users Table.'), count($users)) . '</div>';
		$msg = deleteUsers($users, true);
		print '<div class="form-group row no-gutters cb_form_line">'.$msg.'</div>';
	}

	print '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('Joomla User Table and Joomla Community Builder User Table now in sync!') . '</div>';
...
function checkcbdb( $dbId = 0 ) {
	...
	if ( $dbId == 0 ) {
		...
		echo '<div class="text-left"><div class="form-group row no-gutters cb_form_line">'. CBTxt::T('Checking Community Builder Database') .':</div>';
		...
		if ( $_CB_database->getErrorNum() ) {
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('ERROR: sql query: %s : returned error: %s'), htmlspecialchars( $sql ), stripslashes( $_CB_database->getErrorMsg() )) .  '</div>';
		} elseif (count($bad_rows)!=0) {
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Warning: %s entries in Community Builder comprofiler_field_values have bad fieldid values.'), count($bad_rows)) . '</div>';
	   		foreach ($bad_rows as $bad_row) {
				if ( $bad_row->fieldvalueid == 0 ) {
					echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('ZERO fieldvalueid illegal: fieldvalueid=%s fieldid=0'), $bad_row->fieldvalueid) . '</div>';
				} else {
					echo '"<div class="form-group row no-gutters cb_form_line text-danger">fieldvalueid="' . $bad_row->fieldvalueid . " fieldid=0</div>";
				}
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T('This one can be fixed by <strong>first backing up database</strong>') . ' <a href="' . $_CB_framework->backendUrl( "index.php?option=com_comprofiler&view=fixcbmiscdb&$cbSpoofField=$cbSpoofString" ) . '"> ' . CBTxt::T('then by clicking here') . '</a>.</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('All Community Builder comprofiler_field_values table fieldid rows all match existing fields.') . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Warning: %s entries in Community Builder comprofiler_field_values link back to fields of wrong fieldtype.'), count($bad_rows)) . '</div>';
			foreach ($bad_rows as $bad_row) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">fieldvalueid=' . $bad_row->fieldvalueid . ' fieldtype=' . $bad_row->type .'</div>';
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T('This one can be fixed in SQL using a tool like phpMyAdmin.') . '</div>';
			...
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('All Community Builder comprofiler_field_values table rows link to correct fieldtype fields in comprofiler_field table.') . '</div>';
		}
		...
		if ( $all_comprofiler_fields_and_values === null ) {
			...
			$all_comprofiler_fields		=	array_keys( $all_comprofiler_fields_and_values );
		}
			...
			foreach ( $field_rows as $field_row ) {
				if ( $field_row->tablecolumns !== null ) {
					...
					if ( $field_row->tablecolumns != '' ) {
						...
						foreach ( $tableColumns as $col ) {
							if ( ! in_array( $col, $all_comprofiler_fields ) ) {
								$html_output[]	=	'<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T(' - Field %s - Column %s is missing from comprofiler table.'), $field_row->name, $col) . '</div>';
							}
						}
					}
					$cb11					=	false;
					...
					if ( ! in_array( $field_row->name, $all_comprofiler_fields ) ) {
						$html_output[] = '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T(' - Column %s is missing from comprofiler table.'), $field_row->name) . '</div>';
					}
				}
			}
			if ( count( $html_output ) > 0 ) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('There are %s column(s) missing in the comprofiler table, which are defined as fields (rows in comprofiler_fields):'), count( $html_output )) . '</div>';
				echo implode( '', $html_output );
				echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T('This one can be fixed by deleting and recreating the field(s) using components / Community Builder / Field Management.') . '<br />' . CBTxt::T('Please additionally make sure that columns in comprofiler table <strong>are not also duplicated in users table</strong>.') . '</div>';
			} elseif ( $cb11 ) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T('All Community Builder fields from comprofiler_fields are present as columns in the comprofiler table, but comprofiler_fields table is not yet upgraded to CB 1.2 table structure. Just going to Community Builder Fields Management will fix this automatically.') . '</div>';
			} else {
				echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('All Community Builder fields from comprofiler_fields are present as columns in the comprofiler table.') . '</div>';
			}
		}
		// 9. Check if images/comprofiler is writable:
		$folder = 'images/comprofiler/';
		echo '<div class="form-group row no-gutters cb_form_line">' . CBTxt::T( 'Checking Community Builder folders:' ) . '</div>';
		if ( ! is_writable( $_CB_framework->getCfg('absolute_path'). '/' . $folder ) ) {
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Avatars and thumbnails folder: %s/%s is NOT writeable by the webserver.'), $_CB_framework->getCfg('absolute_path'), $folder) . ' </div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('Avatars and thumbnails folder is Writeable.') . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf( CBTxt::T( 'Warning: %s entries in Community Builder _comprofiler_plugin have bad iscore values.' ), count( $bad_rows ) ) . '</div>';

			foreach ( $bad_rows as $bad_row ) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">plugin=' . $bad_row->name . ' pluginid=' . $bad_row->id . '</div>';
			}

			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T( 'This one can be fixed by <strong>first backing up database</strong>' ) . ' <a href="' . $_CB_framework->backendUrl( "index.php?option=com_comprofiler&view=fixcbdeprecdb&$cbSpoofField=$cbSpoofString" ) . '"> ' . CBTxt::T( 'then by clicking here' ) . '</a>.</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T( 'All Community Builder _comprofiler_plugin table iscore values are correct.' ) . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf( CBTxt::T( 'Warning: %s entries in Community Builder _comprofiler_tabs have bad sys values.' ), count( $bad_rows ) ) . '</div>';

			foreach ( $bad_rows as $bad_row ) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">tab=' . $bad_row->title . ' tabid=' . $bad_row->tabid . '</div>';
			}

			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T( 'This one can be fixed by <strong>first backing up database</strong>' ) . ' <a href="' . $_CB_framework->backendUrl( "index.php?option=com_comprofiler&view=fixcbdeprecdb&$cbSpoofField=$cbSpoofString" ) . '"> ' . CBTxt::T( 'then by clicking here' ) . '</a>.</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T( 'All Community Builder _comprofiler_tabs table sys values are correct.' ) . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf( CBTxt::T( 'Warning: %s entries in Community Builder _comprofiler_fields have bad sys values.' ), count( $bad_rows ) ) . '</div>';

			foreach ( $bad_rows as $bad_row ) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">field=' . $bad_row->title . ' fieldid=' . $bad_row->fieldid . '</div>';
			}

			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T( 'This one can be fixed by <strong>first backing up database</strong>' ) . ' <a href="' . $_CB_framework->backendUrl( "index.php?option=com_comprofiler&view=fixcbdeprecdb&$cbSpoofField=$cbSpoofString" ) . '"> ' . CBTxt::T( 'then by clicking here' ) . '</a>.</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T( 'All Community Builder _comprofiler_fields table sys values are correct.' ) . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf( CBTxt::T( 'Warning: %s entries in Community Builder __comprofiler_plugin are duplicates.' ), count( $bad_rows ) ) . '</div>';
			...
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T( 'All Community Builder __comprofiler_plugin table rows are unique.' ) . '</div>';
		}
		...
		if ( ! $_CB_database->getErrorNum() ) {
			...
			foreach ( $plugins as $plug ) {
				...
				if ( is_bool( $result ) ) {
					...
					echo '<div class="form-group row no-gutters cb_form_line text-warning">' . $dbName . ' "' . $plug->name . '"' . ': ' . $result . '</div>';
				} else {
					echo '<div class="form-group row no-gutters cb_form_line">' . sprintf(CBTxt::T('%s "%s": no database or no database description.'),$dbName ,$plug->name) . '</div>';
				}
			}
		}
		...
		echo '<div class="text-left"><div class="form-group row no-gutters cb_form_line">' . CBTxt::T('Checking Users Database') . ':</div>';
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Warning: %s entries in Community Builder comprofiler table without corresponding user table rows.'), count($bad_rows)) . '</div>';
			$badids	=	array();
			foreach ($bad_rows as $bad_row) {
				$badids[(int) $bad_row->id]	=	$bad_row->id;
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Following comprofiler id: %s are missing in user table'), implode( ', ', $badids )) . ( isset( $badids[0] ) ? " " . CBtxt::T('This comprofiler entry with id 0 should be removed, as it\'s not allowed.') : "" ) . '</div>';
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::Th( 'This one can be fixed using menu Components / Community Builder / tools and then click "Synchronize users".' ) . '</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('All Community Builder comprofiler table rows have links to user table.') . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Warning: %s entries in users table without corresponding comprofiler table rows.'), count($bad_rows)) . '</div>';
			$badids	=	array();
			foreach ($bad_rows as $bad_row) {
				$badids[(int) $bad_row->id]	=	$bad_row->id;
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('users id: %s are missing in comprofiler table'), implode( ', ', $badids )) . '</div>';
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::Th( 'This one can be fixed using menu Components / Community Builder / tools and then click "Synchronize users".' ) . '</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('All users table rows have links to comprofiler table.') . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Warning: %s entries in users table with id=0.'), count($bad_rows)) . '</div>';
			foreach ($bad_rows as $bad_row) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('users id=%s is not allowed.'), $bad_row->id) . '</div>';
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::Th( 'This one can be fixed using menu Components / Community Builder / tools and then click "Synchronize users".' ) . '</div>';
			// echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T('This one can be fixed in SQL using a tool like phpMyAdmin.') . " <strong><u>" . CBTxt::T('You also need to check in SQL if id is autoincremented.') . "<u><strong></font></p>";
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('users table has no zero id row.') . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Warning: %s entries in comprofiler table with id=0.'), count($bad_rows)) . '</div>';
			foreach ($bad_rows as $bad_row) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('comprofiler id=%s is not allowed.'), $bad_row->id) . '</div>';
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::Th('This one can be fixed using menu Components / Community Builder / Tools and then click "Synchronize users".' ) . '</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('comprofiler table has no zero id row.') . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Warning: %s entries in comprofiler table with user_id <> id.'), count($bad_rows)) . '</div>';
			foreach ($bad_rows as $bad_row) {
				echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('comprofiler id=%s is different from user_id=%s.'), $bad_row->id, $bad_row->user_id) . '</div>';
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::Th( 'This one can be fixed using menu Components / Community Builder / tools and then click "Synchronize users".' ) . '</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('All rows in comprofiler table have user_id columns identical to id columns.') . '</div>';
		}
		...
		if ( $_CB_database->getErrorNum() ) {
			...
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('Warning: %s entries in the users table without corresponding user_usergroup_map table rows.'), count($bad_rows)) .'</div>';
			$badids	=	array();
			foreach ($bad_rows as $bad_row) {
				$badids[(int) $bad_row->id]	=	$bad_row->id;
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::T('user id: %s are missing in user_usergroup_map table'), implode( ', ', $badids ));
			echo ( isset( $badids[0] ) ? " " . CBTxt::T('This user entry with id 0 should be removed, as it\'s not allowed.') : "" ) . '</div>';
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::T('This one can be fixed by <strong>first backing up database</strong>') . ' <a href="' . $_CB_framework->backendUrl( "index.php?option=com_comprofiler&view=fixacldb&$cbSpoofField=$cbSpoofString" ) . '">' . CBTxt::T('then by clicking here') . '</a>.</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('All users table rows have ACL entries in user_usergroup_map table.') . '</div>';
		}
		...
		$bad_rows = $_CB_database->loadObjectList();
		if ( $_CB_database->getErrorNum() ) {
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::Th('ERROR: sql query: %s : returned error: %s'), htmlspecialchars( $sql ), stripslashes( $_CB_database->getErrorMsg() )) .  '</div>';
		} elseif (count($bad_rows)!=0) {
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . sprintf(CBTxt::Th('Warning: %s entries in the __user_usergroup_map table without corresponding users table rows.'), count($bad_rows)) . '</div>';
			$badids	=	array();
			foreach ($bad_rows as $bad_row) {
				$badids[(int) $bad_row->id]		=	"user id=" . $bad_row->id;
			}
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::Th( 'DATABASE_CHECK_ENTRIES_OF_TABLE_MISSING_IN_TABLE', 'Following entries of [tablename1] table are missing in [tablename2] table: [badids].', array( '[tablename1]' => 'user_usergroup_map', '[tablename2]' => 'users', '[badids]' => implode( ', ', $badids ))) . ( isset( $badids[0] ) ? "<br /> " . CBTxt::T('This user_usergroup_map entry with (user) value 0 should be removed, as it\'s not allowed.') : "" ) . '</div>';
			echo '<div class="form-group row no-gutters cb_form_line text-danger">' . CBTxt::Th('This one can be fixed by <strong>first backing up database</strong>') . ' <a href="' . $_CB_framework->backendUrl( "index.php?option=com_comprofiler&view=fixacldb&$cbSpoofField=$cbSpoofString" ) . '">' . CBTxt::T('then by clicking here') . '</a>.</div>';
		} else {
			echo '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::Th( 'DATABASE_CHECK_ALL_ENTRIES_OF_TABLE_HAVE_CORRESPONDANCE_IN_TABLE', 'All [tablename1] table rows have corresponding entries in [tablename2] table.', array( '[tablename1]' => 'ACL user_usergroup_map', '[tablename2]' => 'users') ) . '</div>';
		}
...
function fixacldb() {
	...
	$affected		=	$_CB_database->getAffectedRows();
	if ($affected) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf(CBTxt::T('Deleted %s __user_usergroup_map entries which didn\'t correspond to users table.'), $affected) .'</div>';
	}
	...
	$affected		=	$_CB_database->getAffectedRows();
	if ($affected) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf(CBTxt::T('Added %s new entries to __user_usergroup_map table from __user_usergroup_map Table.'),$affected) . '</div>';
	}

	print '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('Joomla User Table and Joomla ACL Table should now be in sync!') . '</div>';
...
function fixcbmiscdb() {
	...
	$affected		=	$_CB_database->getAffectedRows();
	if ($affected) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf(CBTxt::T('Deleted %s comprofiler_field_values entries which didn\'t match any field.'), $affected) . '</div>';
	}
...
*/
	...
	$affected		=	$_CB_database->getAffectedRows();
	if ($affected) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf(CBTxt::T('Deleted %s __comprofiler_plugin duplicate entries.'), $affected) . '</div>';
	}

	print '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T('Misc database adjustments complete!') . '</div>';
...
function fixcbdeprecdb() {
	...
	if ( $affected ) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf( CBTxt::T( 'Fixed %s _comprofiler_plugin entries with incorrect iscore values.' ), $affected ) . '</div>';
	}
	...
	if ( $affected ) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf( CBTxt::T( 'Fixed %s _comprofiler_tabs entries with incorrect sys values.' ), $affected ) . '</div>';
	}
	...
	if ( $affected ) {
		print '<div class="form-group row no-gutters cb_form_line text-warning">' . sprintf( CBTxt::T( 'Fixed %s _comprofiler_fields entries with incorrect sys values.' ), $affected ) . '</div>';
	}
	...
	print '<div class="form-group row no-gutters cb_form_line text-success">' . CBTxt::T( 'Plugin migration complete! Depreciated plugins, tabs, and fields should now be removable!' ) . '</div>';
}
	...
	if ( $task == 'fieldclass' ) {
		...
		if ( ! $iAmAdmin ) {
			if ( Application::MyUser()->isAuthorizedToPerformActionOnAsset( 'core.manage', 'com_users' ) ) {
				if ( $uid == 0 ) {
					$action	=	'core.create';
				} elseif ( $uid == Application::MyUser()->getUserId() ) {
				...
				$iAmAdmin	=	( Application::MyUser()->isAuthorizedToPerformActionOnAsset( $action, 'com_users' )
								&& ( ! Application::User( (int) $uid )->isSuperAdmin() ) );
			}
		}
		...
		$fieldName			=	trim( substr( str_replace( $unsecureChars, '', Application::Input()->get( 'field', null, GetterInterface::COMMAND ) ), 0, 50 ) );
		if ( ! $fieldName ) {
