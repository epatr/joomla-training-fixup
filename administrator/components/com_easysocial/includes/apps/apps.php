...
class SocialApps
{
	...
	public function renderWidgets($group, $view, $position, $args = array(), $wrapper = '')
	{
		...
		$options = array('uid' => $uid, 'widget' => SOCIAL_STATE_PUBLISHED, 'group' => $group, 'limit' => null, 'state' => SOCIAL_STATE_PUBLISHED, 'type' => SOCIAL_APPS_TYPE_APPS, 'sort' => 'element');
		...
		foreach ($apps as $app) {
			...
			if (!method_exists($widget, $position)) {
				continue;
			}
			...
			ob_start();
			call_user_func_array(array($widget, $position), $args);
