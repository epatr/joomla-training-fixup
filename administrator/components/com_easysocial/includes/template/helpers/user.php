...
class ThemesHelperUser extends ThemesHelperAbstract
{
	...
	public function conversation(SocialUser $user, $buttonSize = 'sm')
	{
		...
		$useConverseKit = ES::conversekit()->exists();
	...
	public function friends(SocialUser $user, $buttonSize = 'sm')
	{
		...
		$isFriends = $user->getFriend($this->my->id)->state == SOCIAL_FRIENDS_STATE_FRIENDS;

		// Determines if the friend request has already been sent
		$isPending = $user->getFriend($this->my->id)->state == SOCIAL_FRIENDS_STATE_PENDING;
		...
		if ($isPending) {
			...
			if ($user->getFriend($this->my->id)->actor_id == $this->my->id) {
				$isRequester = true;
			} else {
				$isResponder = true;
			}
