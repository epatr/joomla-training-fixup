...
class SocialRouterVideos extends SocialRouterAdapter
{
	...
	public function build(&$menu, &$query)
	{
		...
		// for users video, we do not want the type and uid segments
		if (isset($query['uid']) && isset($query['type']) && $isUserVideos && !$isSingleVideo) {
			$isUserVideos = true;
			$segments[] = $query['uid'];
		} else {
			$isUserVideos = false;
		...
		if ($menu && ($menu->query['view'] != 'videos' || $isUserVideos || $isClusterVideos)) {
