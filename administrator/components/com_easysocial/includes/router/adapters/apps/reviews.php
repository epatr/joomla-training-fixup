 ...
 * @since	2.0
 */
class SocialRouterAppReviews extends SocialRouterAdapter
{
	/**
	...
	public function build(&$menu , &$query, &$segments)
	{
		...
		if (!is_null($customView)) {
			$segments[]	= $this->translate('reviews_customview_' . $customView );
			unset($query['customView']);
		}

		// Check if user id is supplied. If it does exist, use their alias as the first segment.
		$id = isset($query['reviewId']) ? $query['reviewId'] : null;

		if(!is_null($id)) {
			$segments[]	= $id;
			unset($query['reviewId']);
		}
	...
	public function parse(&$segments, &$vars)
	{
        ...
        $customViews = array(
        	$this->translate('reviews_customview_form'),
        	$this->translate('reviews_customview_edit'),
        	$this->translate('reviews_customview_item')
        );
	...
	{
		if ($translated == $this->translate('reviews_customview_form')) {
			return 'form';
		}

		if ($translated == $this->translate('reviews_customview_edit')) {
			return 'edit';
		}
