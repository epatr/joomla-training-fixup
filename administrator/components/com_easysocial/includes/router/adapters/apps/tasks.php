 ...
 * @since	2.0
 */
class SocialRouterAppTasks extends SocialRouterAdapter
{
	/**
	...
	public function build(&$menu , &$query, &$segments)
	{
		...
		if (!is_null($customView)) {
			$segments[]	= $this->translate('tasks_customview_' . $customView );
			unset($query['customView']);
		}

		// Check if user id is supplied. If it does exist, use their alias as the first segment.
		$id = isset($query['milestoneId']) ? $query['milestoneId'] : null;

		if(!is_null($id)) {
			$segments[]	= $id;
			unset($query['milestoneId']);
		}
	...
	public function parse(&$segments, &$vars)
	{
        ...
        $customViews = array(
        	$this->translate('tasks_customview_form'),
        	$this->translate('tasks_customview_edit'),
        	$this->translate('tasks_customview_item')
        );
	...
	{
		if ($translated == $this->translate('tasks_customview_form')) {
			return 'form';
		}

		if ($translated == $this->translate('tasks_customview_edit')) {
			return 'edit';
		}
