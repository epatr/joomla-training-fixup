 ...
 * @since	2.0
 */
class SocialRouterAppDiscussions extends SocialRouterAdapter
{
	/**
	...
	public function build(&$menu , &$query, &$segments)
	{
		...
		if (!is_null($customView)) {
			$segments[]	= $this->translate('discussions_customview_' . $customView );
			unset($query['customView']);
		}

		// Check if user id is supplied. If it does exist, use their alias as the first segment.
		$id = isset($query['discussionId']) ? $query['discussionId'] : null;

		if(!is_null($id)) {
			$segments[]	= $id;
			unset($query['discussionId']);
		}
	...
	public function parse(&$segments, &$vars)
	{
		...
		$customViews = array(
			$this->translate('discussions_customview_create'),
			$this->translate('discussions_customview_form'),
			$this->translate('discussions_customview_edit'),
			$this->translate('discussions_customview_item')
		);
	...
	{
		if ($translated == $this->translate('discussions_customview_create')) {
			return 'create';
		}

		if ($translated == $this->translate('discussions_customview_form')) {
			return 'form';
		}

		if ($translated == $this->translate('discussions_customview_edit')) {
			return 'edit';
		}
