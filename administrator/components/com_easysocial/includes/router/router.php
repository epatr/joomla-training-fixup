...
abstract class SocialRouterAdapter
{
	...
	public function route($options = array(), $xhtml = true)
	{
		...
		if (!$menuId && $view == 'albums' && !$layout && $uId && $uType == 'user') {
			$menuId = FRoute::getItemId($view, $layout, (int) $uId, $uType, true, true, $lang);
		...
		// normal albums
		if (!$menuId && $view == 'albums' && !$layout && !$id) {
		...
		if ($overrideUrl) {
			...
			if ($menuId){
				...
				$url .= '&Itemid=' . FRoute::getItemId($view, $layout, $id, $type, true, false, $lang);
			}
		}
