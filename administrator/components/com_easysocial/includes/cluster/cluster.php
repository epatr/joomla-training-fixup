...
abstract class SocialCluster
{
	...
	public function allowEvents()
	{
		...
		if (!$this->config->get('events.enabled') || !$my->getAccess()->get('events.create')) {
			return false;
		}
	...
	public function getDefaultAvatar($size)
	{
		...
		if (!isset($defaults[$size])) {
			...
			$overriden = JPATH_ROOT . '/images/easysocial_override/' . $this->cluster_type . '/avatar/' . $size . '.png';
			$uri = ES::getUrl('/images/easysocial_override/' . $this->cluster_type . '/avatar/' . $size . '.png');

			// If avatar override exist for this, use it.
			if (JFile::exists($overriden)) {
				$defaults[$size] = $uri;
			} else {
				$defaults[$size] = ES::getUrl(ES::config()->get('avatars.default.' . $this->cluster_type . '.' . $size));
			}
		}
	...
	public function canCreateFeeds($userId = null)
	{
		...
		if ($this->cluster_type == SOCIAL_TYPE_PAGE && $this->isAdmin()) {
			return true;
		}
