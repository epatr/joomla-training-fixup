...
class SocialStreamItem extends EasySocial
{
	...
	public function bind($data = array())
	{
		...
		$bindable = array('isNew', 'state', 'edited', 'content', 'title', 'params', 'cluster_id', 'cluster_type', 'cluster_access',
						'access', 'custom_access', 'bookmarked', 'sticky', 'modified', 'last_action', 'last_userid', 'last_action_date', 'post_as', 'anywhere_id');
	...
	public function addOgDescription($content)
	{
		...
		$this->meta->setMeta('description', $content);
	...
	public function getPermalink($xhtml = false, $external = false)
	{
		...
		$option = array('id' => $this->uid, 'layout' => 'item');
		if ($external) {
