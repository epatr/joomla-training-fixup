		...
		foreach ($items as &$row) {
			...
			if ($streamItem->comments) {
				if (!$streamItem->comments instanceof SocialCommentBlock) {
					$streamItem->comments = ES::comments($streamItem->contextId, $streamItem->context, $streamItem->verb, $itemGroup, array('url' => FRoute::stream(array('layout' => 'item', 'id' => $streamItem->uid))), $streamItem->uid);
			...
			if ($streamItem->canShare()) {
				$sharingOptions =array('url' => ESR::stream(array('layout' => 'item', 'id' => $streamItem->uid, 'external' => true), true), 'display' => 'dialog', 'text' => JText::_('COM_EASYSOCIAL_STREAM_SOCIAL'), 'css' => 't-fs--sm');
