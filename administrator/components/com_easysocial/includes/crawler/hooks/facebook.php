...
class SocialCrawlerFacebook extends SocialCrawlerAbstract
{
	...
	public function process(&$result)
	{
		...
		if (property_exists($result->oembed, 'height') && !$result->oembed->height) {
			...
			if (isset($contents->picture) && $contents->picture) {
				$result->oembed->thumbnail = $contents->picture;
			} else {
				...
				if ($mainContents) {
					...
					if ($matches && isset($matches[1])) {
						$result->oembed->thumbnail = $matches[1];
					}
				}
			}
		...
		$result->oembed->html = '<iframe src="https://www.facebook.com/plugins/video.php?href=' . $video . '&show_text=false&appId=589470227789490" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true"></iframe>';
	}
}
