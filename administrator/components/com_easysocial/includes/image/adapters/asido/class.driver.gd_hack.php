	...
	function __open(&$tmp) {
		...
		switch(@$i[2]) {
			...
			case 2: // JPG
				...
				$tmp->source = @imageCreateFromJPEG($tmp->source_filename);
				$tmp->target = @imageCreateFromJPEG($tmp->source_filename);

				$error_source = (false == ($tmp->source));
				$error_target = (false == ($tmp->target));
