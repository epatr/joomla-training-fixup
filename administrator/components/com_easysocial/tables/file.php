...
class SocialTableFile extends SocialTable
{
	...
	public function storeWithFile($file)
	{
		...
		if (isset($file['name']) && !empty($file['name'])) {
			...
			// Replace comma to empty because chrome browser will restricted this when you trying to download the file.
			$this->name = str_replace(',', '', $filename);
		}
	...
	public function getPreviewURI()
	{
		...
		$uri = ESR::raw('index.php?option=com_easysocial&view=' . $type . '&layout=preview&fileid=' . $this->id . '&tmpl=component');
