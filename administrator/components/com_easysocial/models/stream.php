...
class EasySocialModelStream extends EasySocialModel
{
	...
	private function getStreamTableAlias($userId, $type, $userStickyOnly = false, $useDate = false, $direction = '', $startdate = null, $enddate = null, $customView = false, $context = SOCIAL_STREAM_CONTEXT_TYPE_ALL, $anywhereId = false)
	{
		...
		// if ((ES::config()->get('stream.filter.apps') == 'me' && ES::config()->get('friends.enabled')) ||
		// 	( ES::config()->get('stream.filter.apps') == 'me' && !ES::config()->get('friends.enabled') && $context == SOCIAL_STREAM_CONTEXT_TYPE_ALL)) {

		if ((ES::config()->get('friends.enabled')) ||
			(!ES::config()->get('friends.enabled') && $context == SOCIAL_STREAM_CONTEXT_TYPE_ALL)) {
			// if friends system enabled, we only want to retrieve streams belong to current users and their friends.
	...
	public function getStreamData($config = array())
	{
		...
		$customView = ($customView) ? $customView : JRequest::getVar('view', '');
		...
		if (($sysconfig->get('stream.filter.apps') == 'me' || $context == 'all')
			&& empty($listid) && empty($isBookmark) && empty($isFollow) && empty($profileId) && !$streamId && !$guest && !$tag && !$isAdmin && !$isAdminSticky) {
		...
		if ($isSticky && $userStickyOnly && !$includeClusterSticky) {
			...
			$cond[]	= '(a.`cluster_id` > 0 and (a.`cluster_access` = 1) OR (a.`cluster_access` = 4))';
	...
	public function updateAccess($streamId, $privacy, $custom = null, $field = null)
	{
		...
		$query .= ', `field_access` = ' . $db->Quote(count($field));
