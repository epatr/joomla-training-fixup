...
class EasySocialModelLeaderboard extends EasySocialModel
{
	...
	public function getLadder($options = array() , $loadUsers = true)
	{
		...
		$query = array();
		$query[] = 'SELECT a.' . $db->nameQuote('id') . ', SUM(d.' . $db->nameQuote('points') . ') AS ' . $db->nameQuote('points');
		...
		if ($recentdays) {
			$now = ES::date()->toSql();
			$query[] = 'AND (d.`created` <= ' . $db->Quote($now) . ' AND d.`created` >= date_sub(' . $db->Quote($now) . ', INTERVAL ' . $recentdays . ' DAY))';
		}
