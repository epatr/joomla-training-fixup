...
class EasySocialModelPhotos extends EasySocialModel
{
	...
	public function getModulePhotos($options = array())
	{
		...
		$accessFieldColumnAlbum = "(select `field_access` from `#__social_privacy_items` as pri where pri.`uid`=a.`album_id` and pri.`type` = 'albums' limit 1) as field_accessAlbum";
	...
	public function getPhotos($options = array())
	{
		...
		if (!empty($ordering)) {
			...
			if ($ordering == 'created') {
				$query[] = "order by $tblPrefix.`created` $sort";
			}

			if ($ordering == 'ordering') {
				$query[] = "order by $tblPrefix.`ordering` $sort";
			}

			if ($ordering == 'album_id') {
				$query[] = "order by `album_id` $sort";
			}
	...
	private function getAccessColumn($type = 'access', $prefix = 'a')
	{
		...
		if ($type == 'access') {
			...
			$column .= " left join `#__social_privacy_customize` as prc on pri.id = prc.uid and prc.utype = 'item' where pri.uid = " . $prefix . ".id and pri.`type` = 'photos'";
			$column .= " UNION ALL ";
			...
			$column .= "  and pp.type = 'photos' and pp.rule = 'view'";
			$column .= " union all ";
			$column .= " select prm.value as `access`";
			$column .= " from `#__social_privacy_map` as prm";
			$column .= "  inner join `#__social_privacy` as pp on prm.privacy_id = pp.id";
			...
			$column = "(select `field_access` from `#__social_privacy_items` as pri where pri.`uid`= " . $prefix .".`id` and pri.`type` = 'photos' limit 1) as field_access";
		}
