...
class EasySocialModelApps extends EasySocialModel
{
	 ...
	 * @access	public
	 */
	public function getGroupApps($groupId, $respectView = false)
	{
		$db = ES::db();
		$sql = $db->sql();
		...
		foreach ($result as $row) {
			...
			$canAccess = $app->hasAccess($group->getCategory()->id);
	 ...
	 * @return
	 */
	public function getPageApps($pageId, $respectView = false)
	{
		$db = ES::db();
		$sql = $db->sql();
		...
		foreach ($result as $row) {
			...
			$page = ES::page($pageId);
			$canAccess = $app->hasAccess($page->getCategory()->id);
	 ...
	 * @access	public
	 */
	public function getEventApps($eventId, $respectView = false)
	{
		$db = ES::db();
		$sql = $db->sql();
		...
		foreach ($result as $row) {
			...
			$canAccess = $app->hasAccess($event->getCategory()->id);
