...
class EasySocialModelUsers extends EasySocialModel
{
	...
	public function getUsers($options = array())
	{
		...
		// determine if we only want to get users who are friends.
		...
		if (ES::config()->get('users.blocking.enabled') && $excludeBlocked && !JFactory::getUser()->guest) {
			...
			$query .= " LEFT JOIN `#__social_block_users` as bus on a.`id` = bus.`user_id`";
			$query .= " 	AND bus.`target_id` = " . $db->Quote($my->id);
		}
		...
		if ($registrationState) {
			if ($registrationState == 'pending') {
				$wheres[] = "(b.state = " . $db->Quote(SOCIAL_REGISTER_VERIFY) . " OR b.state = " . $db->Quote(SOCIAL_REGISTER_APPROVALS) . ")";
			} else {
		...
		if ($includeAdmin === false) {
			...
			if ($admins) {
				...
				$wheres[] = "a.id NOT IN (" . implode(',', $ids) . ")";
			}
		}
	...
	public function create($data, SocialUser $user, SocialTableProfile $profile, $forceApprove = false)
	{
		...
		if (($type == 'approvals' || $type == 'verify') && !$forceApprove) {
			$user->block = 1;
		}
