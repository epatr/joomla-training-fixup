...
class EventbookingModelCommonRegistrant extends RADModelAdmin
{
	...
	public function store($input, $ignore = array())
	{
		...
		if ($data['id'])
		{
			...
			{
				$rowFields = EventbookingHelperRegistration::getFormFields($data['event_id'], 1, $row->language);
			}
			else
			{
				$rowFields = EventbookingHelperRegistration::getFormFields($data['event_id'], 0, $row->language);
			}
			...
			*/
			...
			if ($row->is_group_billing && ($config->collect_member_information || $config->collect_member_information_in_cart))
			{
				$ids              = (array) $data['ids'];
	...
	public function sendCertificates($id)
	{
		...
		EventbookingHelperMail::sendCertificateEmail($row, $config);
	}
