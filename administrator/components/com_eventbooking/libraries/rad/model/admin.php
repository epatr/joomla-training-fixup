...
class RADModelAdmin extends RADModel
{
	...
	public function store($input, $ignore = array())
	{
		...
		if ($this->triggerEvents)
		{
			$result = $app->triggerEvent($this->eventBeforeSave, array($row, $data, $isNew));
		...
		if ($this->triggerEvents)
		{
			$app->triggerEvent($this->eventAfterSave, array($row, $data, $isNew));
		}
	...
	public function publish($pks, $value = 1)
	{
		...
		if ($this->triggerEvents)
		{
			...
			JPluginHelper::importPlugin($this->pluginGroup);
			JFactory::getApplication()->triggerEvent($this->eventChangeState, array($this->context, $pks, $value));
		}
