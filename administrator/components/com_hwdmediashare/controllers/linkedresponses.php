...
class hwdMediaShareControllerLinkedResponses extends JControllerAdmin
{
	...
	public function unlink()
	{
		...
		else
		{
			...
			{
				$this->setMessage(JText::plural($this->text_prefix . '_N_RESPONSES_UNLINKED_FROM_MEDIA', count($cid)));
			}
			else
			{
	...
	public function link()
	{
		...
		else
		{
			...
			{
				$this->setMessage(JText::plural($this->text_prefix . '_N_RESPONSES_LINKED_TO_MEDIA', count($cid)));
			}
			else
			{
				$this->setMessage($model->getError());
