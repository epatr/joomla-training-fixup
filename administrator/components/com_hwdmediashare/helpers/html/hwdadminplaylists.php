...
abstract class JHtmlHwdAdminPlaylists
{
	...
	public static function status($value = 0, $i, $canChange = true)
	{
		...
		$states	= array(
			...
			2	=> array('pending',    'playlists.approve',    'COM_HWDMS_PENDING',     'COM_HWDMS_TOGGLE_TO_APPROVE'),
			3	=> array('not-ok',     'playlists.approve',    'COM_HWDMS_REPORTED',    'COM_HWDMS_TOGGLE_TO_APPROVE'),
		);
		$state	= JArrayHelper::getValue($states, (int) $value, $states[1]);
		$icon	= $state[0];
