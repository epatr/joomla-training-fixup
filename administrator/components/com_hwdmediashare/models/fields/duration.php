	...
	protected function getInput()
	{
		...
		$doc = JFactory::getDocument();            
                ...
                jQuery(document).ready(function(){
                    jQuery('#durationpicker').timepicker('setTime', '<?php echo $duration->h . ':' . $duration->m . ':' . $duration->s; ?>');
                    jQuery('#durationpicker').timepicker().on('changeTime.timepicker', function(e) {
                        var duration = (60*60*e.time.hours) + (60*e.time.minutes) + (e.time.seconds);
                        console.log('The duration is ' + duration);
                        jQuery('#jform_duration').val(duration);
                    })
                    jQuery('#durationpicker').timepicker().on('hide.timepicker', function(e) {
                        jQuery('#durationpicker').css('display' , 'inline-block');
                    });
                });           
                </script>
                <?php
