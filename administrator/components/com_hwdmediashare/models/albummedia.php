...
class hwdMediaShareModelAlbumMedia extends JModelList
{
	...
	public function getItems()
	{
                ...
                if ($table->load($app->input->get('album_id', '', 'int')))
                {          
                        $properties = $table->getProperties(1);
                        $album = JArrayHelper::toObject($properties, 'JObject');
                        $this->model->setState('filter.author_id', intval($album->created_user_id));
                        $this->model->setState('filter.author_id.include', 1);
