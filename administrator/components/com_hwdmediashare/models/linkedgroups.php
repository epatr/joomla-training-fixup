...
{
	/**
	 * The model used to get the list of groups.
         * 
         * @access  protected
	 * @var     object
	 */    
	...
	public function getItems()
	{
                ...
                $this->model = JModelLegacy::getInstance('Groups', 'hwdMediaShareModel', array('ignore_request' => true));
                $this->model->populateState();
                $this->model->setState('filter.add_to_media', $app->input->get('add', '0', 'int'));
                $this->model->setState('filter.media_id',  $app->input->get('media_id', '', 'int'));
	...
	protected function populateState($ordering = null, $direction = null)
	{
                ...
                $this->setState('filter.add_to_media', $app->input->get('add', '0', 'int'));
                $this->setState('filter.media_id', $app->input->get('media_id', '', 'int'));
	...
	/**
	 * Method to unlink one or more groups from a media.
	 *
	 * @access  public
	 ...
	 */
		...
		foreach ($pks as $i => $pk)
		{
                        ...
                        foreach ($rows as $x => $row)
                        {
                                if ($table->load($row))
                                {
                                        if ($HWDauthorise->authoriseGroupAction('unlink', $pk, $mediaId))
                                        {
                                                if (!$table->delete($row))
                                                {
	...
	/**
	 * Method to link one or more groups to a media.
	 *
	 * @access  public
	 * @param   array    $pks      A list of the primary keys to change.
	 * @param   integer  $mediaId  The value of the media key to associate with.
	...
	public function link($pks, $mediaId = null)
	{
		...
		foreach ($pks as $i => $pk)
		{
			$table->reset();
                        ...
                        if (!$HWDauthorise->authoriseGroupAction('link', $pk, $mediaId))
                        {
                                // Prune items that you can't change.
                                unset($pks[$i]);
                                $error = $this->getError();
