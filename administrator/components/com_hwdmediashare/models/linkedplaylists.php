...
class hwdMediaShareModelLinkedPlaylists extends JModelList
{
	...
	public function getItems()
	{
                ...
                $this->model = JModelLegacy::getInstance('Playlists', 'hwdMediaShareModel', array('ignore_request' => true));
	...
	/**
	 * Method to unlink one or more playlists from a media.
	 *
	 * @access  public
	 ...
	 */
		...
		foreach ($pks as $i => $pk)
		{
                        ...
                        foreach ($rows as $x => $row)
                        {
                                if ($table->load($row))
                                {
                                        if ($HWDauthorise->authorisePlaylistAction('unlink', $pk, $mediaId))
                                        {
                                                if (!$table->delete($row))
                                                {
	...
	/**
	 * Method to link one or more playlists to a media.
	 *
	 * @access  public
	 * @param   array    $pks      A list of the primary keys to change.
	 * @param   integer  $mediaId  The value of the media key to associate with.
	...
	public function link($pks, $mediaId = null)
	{
		...
		foreach ($pks as $i => $pk)
		{
			$table->reset();
                        ...
                        if (!$HWDauthorise->authorisePlaylistAction('link', $pk, $mediaId))
                        {
                                // Prune items that you can't change.
                                unset($pks[$i]);
                                $error = $this->getError();
