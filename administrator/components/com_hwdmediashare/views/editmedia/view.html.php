...
class hwdMediaShareViewEditMedia extends JViewLegacy 
{
	...
	protected function addToolBar()
	{
		...
		if ($this->item->type == 6)
		{
                        ...
                        $syncToPlatform = 'COM_HWDMS_SYNC_TO_PLATFORM';
                        $syncFromPlatform = 'COM_HWDMS_SYNC_FROM_PLATFORM';
                        ...
                        if (file_exists($pluginPath))
                        {
                                ...
                                if ($lang->load($plugin, JPATH_ADMINISTRATOR, $lang->getTag()))
                                {
                                        $syncToPlatform = 'PLG_HWDMEDIASHARE_' . $this->item->storage . '_SYNC_TO_PLATFORM';
                                        $syncFromPlatform = 'PLG_HWDMEDIASHARE_' . $this->item->storage . '_SYNC_FROM_PLATFORM';
                                }
                        }

                        JToolBarHelper::custom('editmedia.syncToPlatform', 'arrow-up', 'arrow-up', $syncToPlatform, false);
			JToolBarHelper::custom('editmedia.syncFromPlatform', 'arrow-down', 'arrow-down', $syncFromPlatform, false);
		}                
