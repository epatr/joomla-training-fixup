...
$canCreate  = $user->authorise('core.create',     'com_hwdmediashare.playlist');
$canEdit    = $user->authorise('core.edit',       'com_hwdmediashare.playlist.'.$item->id);
$canCheckin = $user->authorise('core.manage',     'com_hwdmediashare') || $item->checked_out == $userId || $item->checked_out == 0;
$canEditOwn = $user->authorise('core.edit.own',   'com_hwdmediashare.playlist.'.$item->id) && $item->created_user_id == $userId;
$canChange  = $user->authorise('core.edit.state', 'com_hwdmediashare.playlist.'.$item->id) && $canCheckin;
?>
<tr class="row<?php echo $i % 2; ?>">
        ...
        <td class="center">
                <div class="btn-group">
                        ...
                        JHtml::_('actionsdropdown.' . $action, 'cb' . $i, 'playlists');
        ...
        <td class="center">
                <a href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&view=reported&layout=playlists&tmpl=component&id=' . $item->id); ?>" class="modal" rel="{handler: 'iframe', size: {x: 800, y: 500}}">
        ...
        <td class="nowrap has-context">
                <div class="pull-left">
                        ...
                        <?php if ($canEdit || $canEditOwn) : ?>
                                <a href="<?php echo JRoute::_('index.php?option=com_hwdmediashare&task=playlist.edit&id=' . $item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
