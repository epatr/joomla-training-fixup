...
class modMediaCategoriesHelper extends JViewLegacy 
{
	...
	public function __construct($module, $params)
	{
		...
		hwdMediaShareFactory::load('utilities');
                ...
                $this->startLevel = $this->items ? reset($this->items)->getParent()->level : 0;
	...
	public function getItems()
	{
		...
		$options['module']     = true; // Dummy option array to load a complete category node tree.
                ...
                // Check if we need to extract current category.
                if ($this->params->get('parent', 'root') == 'dynamic')
                {
                        if ($app->input->get('option') == 'com_hwdmediashare' && $app->input->get('view') == 'category' && $app->input->get('id') > 0)
                        {
                                $this->params->set('parent', $app->input->get('id', 'root'));
                        }
                        elseif ($app->input->get('option') == 'com_hwdmediashare' && $app->input->get('category_id') > 0)
                        {
                                $this->params->set('parent', $app->input->get('category_id', 'root'));
                        }                        
                }
