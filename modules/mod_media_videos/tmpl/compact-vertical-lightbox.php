...
<div class="hwd-container">
        ...
        <div class="row-fluid">
          ...
          <div class="span12">
            ...
            <div class="media-item<?php echo ($helper->params->get('list_thumbnail_aspect') == 0 ? ' originalaspect' : ''); ?><?php echo ($helper->params->get('list_tooltip_location') > '2' ? ' hasTooltip' : ''); ?>" title="<?php echo ($helper->params->get('list_tooltip_location') > '1' ? JHtml::tooltipText($item->title, ($helper->params->get('list_tooltip_contents') != '0' ? JHtml::_('string.truncate', $item->description, $helper->params->get('list_desc_truncate'), false, false) : '')) : $item->title); ?>">
              ...
              <?php endif; ?>
              <?php if ($helper->params->get('list_link_thumbnails') == 1) :?>
                <?php echo JHtml::_('HwdPopup.link', $item, '<img src="' . JRoute::_(hwdMediaShareThumbnails::thumbnail($item)) . '" border="0" alt="' . $helper->escape($item->title) . '" class="media-thumb" />'); ?>                                      
              <?php else: ?>
            ...
            <?php if ($helper->params->get('list_meta_title') != '0') :?>
              <h<?php echo $helper->params->get('list_item_heading'); ?> class="contentheading<?php echo ($helper->params->get('list_tooltip_location') > '1' ? ' hasTooltip' : ''); ?>" title="<?php echo ($helper->params->get('list_tooltip_location') > '1' ? JHtml::tooltipText($item->title, ($helper->params->get('list_tooltip_contents') != '0' ? JHtml::_('string.truncate', $item->description, $helper->params->get('list_desc_truncate'), false, false) : '')) : $item->title); ?>">
                <?php if ($helper->params->get('list_link_titles') == 1) :?>
                  <?php echo JHtml::_('HwdPopup.link', $item, $helper->escape(JHtml::_('string.truncate', $item->title, $helper->params->get('list_title_truncate'), false, false))); ?>
                <?php else: ?>
                  <?php echo JHtml::_('string.truncate', $item->title, $helper->params->get('list_title_truncate'), false, false); ?>
                <?php endif; ?>
              </h<?php echo $helper->params->get('list_item_heading'); ?>>
            <?php endif; ?>         
            <?php if ($item->featured && $helper->params->get('list_meta_featured_status') != '0'): ?>
  ...
  <?php if ($params->get('show_more_link') != 'hide') :?><p><a href="<?php echo ((intval($params->get('show_more_link')) > 0) ? JRoute::_($menu->getItem($params->get('show_more_link'))->link.'&Itemid='.$params->get('show_more_link')) : JRoute::_(hwdMediaShareHelperRoute::getMediaRoute())); ?>" class="btn"><?php echo JText::_($params->get('more_link_text', 'MOD_MEDIA_VIDEOS_VIEW_MORE')); ?></a></p><?php endif; ?>  
</div>
