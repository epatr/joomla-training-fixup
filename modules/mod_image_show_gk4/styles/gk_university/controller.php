...
class GKIS_gk_university_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		...
		if($this->config['generate_thumbnails'] == 1) {
			// basic images params		
			$img_width = $this->config['config']->gk_university->gk_university_image_width;
			$img_height = $this->config['config']->gk_university->gk_university_image_height;
			$img_bg = $this->config['config']->gk_university->gk_university_image_bg;
			$quality = $this->config['config']->gk_university->gk_university_quality;
			// check the slides
			foreach($this->config['image_show_data'] as $slide) {
				$stretch = ($slide->stretch == 'nostretch') ? false : true;
				GKIS_University_Image::createThumbnail($slide->image, $this->config, $img_width, $img_height, $img_bg, $stretch, $quality);	
			}
		}
	}
	// get the articles data
	...
	function generateView() {
		...
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "anim_speed": '.$this->config['config']->gk_university->gk_university_animation_speed.', "anim_interval": '.$this->config['config']->gk_university->gk_university_animation_interval.', "slide_links": '.$this->config['config']->gk_university->gk_university_slide_links.' };');
		// generate necessary variables
		$width = $this->config['config']->gk_university->gk_university_image_width;
		$height = $this->config['config']->gk_university->gk_university_image_height;
		$t_width = $this->config['config']->gk_university->gk_university_tablet_image_width;
		$t_height = $this->config['config']->gk_university->gk_university_tablet_image_height;
		$m_width = $this->config['config']->gk_university->gk_university_mobile_image_width;
		$m_height = $this->config['config']->gk_university->gk_university_mobile_image_height;
		// add styles fragment
		$document->addStyleDeclaration('
		#gkIs-'.$this->config['module_id'].'.gkIsWrapper-gk_university {
			min-height: '.$height.'px;
		}
		
		#gkIs-'.$this->config['module_id'].'.gkIsWrapper-gk_university .gkIsSlide {
			background-size: '.$width.'px '.$height.'px;
			height: '.$height.'px;
			max-width: '.$width.'px;
		}
		
		#gkIs-'.$this->config['module_id'].'.gkIsWrapper-gk_university figure,
		#gkIs-'.$this->config['module_id'].'.gkIsWrapper-gk_university .gkIsPreloader {
			max-width: '.$width.'px;
		}
