...
if(isJoomla4()) {
  include_once(JPATH_SITE . DS . 'modules' . DS . 'mod_image_show_gk4' . DS . 'admin' . DS . 'elements' . DS . 'j4' . DS . 'slidemanager.php');
} else {
  class JFormFieldSlidemanager extends JFormField {
  	...
  	protected function getInput() {
  		...
  		$textarea = '<textarea name="'.$this->name.'" id="'.$this->id.'" rows="20" cols="50">'.$option_value.'</textarea>';
  	...
  	private function getForm($type = 'add') {
         ...
         // href="index.php?option=com_k2&amp;view=items&amp;task=element&amp;tmpl=component&amp;object=jform_artK2"
          ...
          // form_buttons
          $form_buttons = '<div class="gk_tab_'.$type.'_submit"><a href="#save" class="btn btn-success">'.JText::_('MOD_IMAGE_SHOW_GK4_FORM_SAVE').'</a><a href="#cancel" class="btn">'.JText::_('MOD_IMAGE_SHOW_GK4_FORM_CANCEL').'</a></div>';
