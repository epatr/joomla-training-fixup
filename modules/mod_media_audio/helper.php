...
class modMediaAudioHelper extends JViewLegacy
{
	/**
	 * Class constructor.
	 *
	...
	{
                // Load caching.
                $cache = JFactory::getCache('mod_media_audio');
                $cache->setCaching(1);  
	...
	public function addHead()
	{
		...
		$doc = JFactory::getDocument();
                ...
                {
                        $doc->addStyleSheet(JURI::root() . 'modules/mod_media_audio/slick/slick.css');
                        $doc->addScript(JURI::root() . 'modules/mod_media_audio/slick/slick.min.js');
...
jQuery(document).ready(function(){
    jQuery('#media-carousel-view-" . $this->module->id . "').slick({
      ...
      speed: " . $this->params->get('speed', 300) . ",
	...
	public function getItems()
	{
                        ...
                        case 2:
                                ...
                                if ($catids)
                                {
                                        if ($this->params->get('show_child_category_articles', 0) && (int) $this->params->get('levels', 0) > 0) 
                                        {
                                                ...
                                                require_once(JPATH_ROOT.'/components/com_hwdmediashare/models/categories.php'); // Model sometimes not available (#73980) so require it.
		...
		// Additional filters.
                $model->setState('filter.media_type', 1);                
