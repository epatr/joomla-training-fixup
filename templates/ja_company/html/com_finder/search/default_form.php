				...
				if ($this->params->get('show_autosuggest', 1))
				{
					...
					$script .= "
						...
						var suggest = jQuery('#q').autocomplete({
							...
							zIndex: 9999,
							deferRequestBy: 500,
							onSearchComplete: function(_q, _s){
								if (jQuery('html').attr('dir') == 'rtl') {
									if (_r == '')
										_r = jQuery('.autocomplete-suggestions').css('left');
									jQuery('.autocomplete-suggestions').css('right', _r);
								}
							}
					});";
				}
...
<form id="finder-search" action="<?php echo JRoute::_($this->query->toUri()); ?>" method="get" class="js-finder-searchform form-inline">
	...
	<fieldset class="word">
		<div class="form-group">
			...
			<input type="text" name="q" id="q" size="30" value="<?php echo $this->escape($this->query->input); ?>" class="js-finder-search-query inputbox" />
		</div>
		<div class="form-group">
