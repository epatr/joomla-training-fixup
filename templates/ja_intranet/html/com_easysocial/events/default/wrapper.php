...
<div class="btn-group btn-group-justified es-btn-groub--p-date t-lg-mxl">
...
<div class="es-cards">
	<?php foreach($featuredEvents as $event){ ?>
		<?php echo $this->loadTemplate('site/events/default/item', array('event' => $event, 'showDistance' => $showDistance, 'isGroupOwner' => isset($isGroupOwner) ? $isGroupOwner : false, 'showSidebar' => $showSidebar)); ?>
	<?php } ?>
</div>
<?php } ?>

<div>
	<?php if (!$delayed && ($showPastFilter || $showSorting || $showDistanceSorting) && $showSidebar) { ?>
	<div class="o-grid">
					...
					<div class="o-form-group">
						...
						<div class="o-grid__cell">
							<div class="es-list-sorting">
								<?php echo $this->html('form.popdown', 'radius', '10', $sortOptions); ?>
							</div>
						</div>
					</div>
					<?php } ?>
		...
		<?php if ($showSorting || $showDistanceSorting) { ?>
			<?php
				...
				$sortOptions[] = $this->html('form.popdownOption', 'start', 'COM_EASYSOCIAL_EVENTS_SORTING_EVENT_DATE', '', false, array('data-ordering="start"'), $url);

				if ($showSorting) {
					$url = $sortingUrls['created'][$showPastFilter && $includePast ? 'past' : 'nopast'];
					$sortOptions[] = $this->html('form.popdownOption', 'created', 'COM_EASYSOCIAL_EVENTS_SORTING_EVENT_CREATED_DATE', '', false, array('data-ordering="created"'), $url);
				}
			...
			<div class="o-grid__cell">
				<div class="es-list-sorting">
					<?php echo $this->html('form.popdown', 'sorting', 'start', $sortOptions); ?>
				</div>
			</div>
