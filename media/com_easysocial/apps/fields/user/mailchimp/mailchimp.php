...
class SocialFieldsUserMailchimp extends SocialFieldItem
{
	...
	public function onRegisterAfterSave(&$data, $user)
	{
		...
		if ($subscribe && $defaultList && $params->get('apikey')) {
			...
			// Determine if there enable welcome email setting
			$welcomeEmail = $fieldParams->get('welcome_email', true);
			...
			$mailchimp->subscribe($listId, $user->email, $firstName, $lastName, $welcomeEmail, $useDoubleOptIn);
		}
	}
	...
	public function onEditAfterSaveFields(&$data, SocialUser $user)
	{
		...
		if ($subscribe && $defaultList && $params->get('apikey')) {
			...
			// Determine if there's enable welcome email setting
			$welcomeEmail = $fieldParams->get('welcome_email', true);
