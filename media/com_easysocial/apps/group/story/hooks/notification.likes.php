...
class SocialGroupAppStoryHookNotificationLikes extends SocialAppHooks
{
	public function execute(&$item)
	{
		...
		// When someone likes a post in a group
		if ($item->context_type == 'story.group.create') {
			...
			if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$item->title = JText::sprintf($this->getPlurality('APP_GROUP_STORY_USER_LIKES_YOUR_POST', $users), $names, $group->getName());
				return;
			}

			// This is for 3rd party viewers
			$item->title = JText::sprintf($this->getPlurality('APP_GROUP_STORY_USER_LIKES_USERS_POST', $users), $names, ES::user($stream->actor_id)->getName(), $group->getName());

			return;
		}

		// When someone likes a link in a group
		if ($item->context_type == 'links.create') {
			...
			// We need to generate the notification message differently for the author of the item and the recipients of the item.
			if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$item->title = JText::sprintf($this->getPlurality('APP_GROUP_STORY_USER_LIKES_YOUR_LINK', $users), $names, $group->getName());
				return;
			}

			$item->title = JText::sprintf($this->getPlurality('APP_GROUP_STORY_USER_LIKES_USERS_LINK', $users), $names, ES::user($stream->actor_id)->getName(), $group->getName());
	...
	private function notificationPhotos($names, $users, &$item)
	{
		...
		// We need to generate the notification message differently for the author of the item and the recipients of the item.
		if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
			$item->title = JText::sprintf($this->getPlurality('APP_GROUP_STORY_USER_LIKES_YOUR_SHARED_PHOTO', $users), $names, $group->getName());
			return;
		}

		$item->title = JText::sprintf($this->getPlurality('APP_GROUP_STORY_USER_LIKES_USERS_SHARED_PHOTO', $users), $names, ES::user($stream->actor_id)->getName(), $group->getName());
		return;
	}
}
