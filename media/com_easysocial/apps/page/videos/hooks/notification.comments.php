    ...
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'videos.page.create') {
            ...
            if ($item->actor_id == $video->user_id && count($users) == 1) {
                ...
                $item->title = JText::sprintf('APP_USER_VIDEOS_NOTIFICATIONS_COMMENTED_ON_USERS_VIDEO' . ES::user($item->actor_id)->getGenderLang(), ES::user($item->actor_id)->getName());

                return;
            }

            // For other users, we just post a generic message
            $item->title = JText::sprintf('APP_USER_VIDEOS_NOTIFICATIONS_COMMENTED_ON_USERS_VIDEO', $names, ES::user($video->user_id)->getName());
