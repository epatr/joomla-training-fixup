...
class SocialUserAppBadges extends SocialAppItem
{
	...
	public function onPrepareStream( SocialStreamItem &$item )
	{
		...
		$item->comments = ES::comments($item->contextId , $item->context, $item->verb . '.' . $item->actor->id , SOCIAL_APPS_GROUP_USER , array( 'url' => FRoute::stream( array( 'layout' => 'item', 'id' => $item->uid ) ) ), $item->uid );
