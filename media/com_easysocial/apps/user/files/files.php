...
class SocialUserAppFiles extends SocialAppItem
{
	...
	public function onPrepareStream(SocialStreamItem &$item, $includePrivacy = true )
	{
		...
		$item->comments = ES::comments($item->uid, $item->context, $item->verb, SOCIAL_APPS_GROUP_USER, array('url' => $item->getPermalink()), $item->uid);
	...
	public function onPrepareStoryPanel($story, $isEdit = false, $data = array())
	{
		...
		if (!$access->get('files.upload') || !$params->get('enable_uploads', true) || !$this->getApp()->hasAccess($this->my->profile_id)) {
			return;
		}
