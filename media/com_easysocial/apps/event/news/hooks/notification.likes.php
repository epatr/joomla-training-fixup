...
class SocialEventAppNewsHookNotificationLikes extends SocialAppHooks
{
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'news.event.create') {
			...
			if ($news->created_by == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$item->title = JText::sprintf($this->getPlurality('APP_EVENT_NEWS_USER_LIKES_YOUR_ANNOUNCEMENT', $users), $names, $event->getName());

				return;
			}

			// This is for 3rd party viewers
			$item->title = JText::sprintf($this->getPlurality('APP_EVENT_NEWS_USER_LIKES_USER_ANNOUNCEMENT', $users), $names, ES::user($news->created_by)->getName(), $event->getName());
