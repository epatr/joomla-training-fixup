...
class SocialEventAppPhotos extends SocialAppItem
{
	...
	private function processActions(SocialStreamItem &$item)
	{
		...
		if ($item->verb =='share') {
			...
			$comments = ES::comments($objectId, $objectType, 'upload', SOCIAL_APPS_GROUP_EVENT,  array('url' => $commentUrl), $item->uid);
		...
		if (count($item->contextIds) > 1) {
			...
			$comments = ES::comments($photo->album_id, 'albums', 'create', SOCIAL_APPS_GROUP_GROUP, $commentParams, $item->uid);
			$item->comments = $comments;
