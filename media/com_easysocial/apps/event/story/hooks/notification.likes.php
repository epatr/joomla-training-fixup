...
class SocialEventAppStoryHookNotificationLikes extends SocialAppHooks
{
	/**
	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'story.event.create') {
			...
			{
				$item->title = JText::sprintf($this->getPlurality('APP_EVENT_STORY_USER_LIKES_YOUR_POST', $users), $names, $event->getName());

				return;
			}

			// This is for 3rd party viewers
			$item->title = JText::sprintf($this->getPlurality('APP_EVENT_STORY_USER_LIKES_USERS_POST', $users), $names, ES::user($stream->actor_id)->getName(), $event->getName());
		...
		if ($item->context_type == 'links.create') {
			...
			if($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER)
			{
				$item->title = JText::sprintf($this->getPlurality('APP_EVENT_STORY_USER_LIKES_YOUR_LINK', $users), $names, $event->getName());

				return;
			}

			// This is for 3rd party viewers
			$item->title = JText::sprintf($this->getPlurality('APP_EVENT_STORY_USER_LIKES_USERS_LINK', $users), $names, ES::user($stream->actor_id)->getName(), $event->getName());
	...
	private function notificationPhotos($names, $users, &$item)
	{
		...
		if($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER)
		{
			$item->title = JText::sprintf($this->getPlurality('APP_EVENT_STORY_USER_LIKES_YOUR_SHARED_PHOTO', $users), $names, $event->getName());

			return;
		}

		// This is for 3rd party viewers
		$item->title = JText::sprintf($this->getPlurality('APP_EVENT_STORY_USER_LIKES_USERS_SHARED_PHOTO', $users), $names, ES::user($stream->actor_id)->getName(), $event->getName());
