...
class SocialEventAppFilesHookNotificationLikes extends SocialAppHooks
{
	/**
	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'files.event.uploaded') {
			...
			// We need to generate the notification message differently for the author and recipients of the item.
			if ($file->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER)
			{
				$item->title = JText::sprintf($this->getPlurality('APP_EVENT_FILES_USER_LIKES_YOUR_FILE', $users), $names);

				return;
			}

			// This is for 3rd party viewers
			$item->title = JText::sprintf($this->getPlurality('APP_EVENT_FILES_USER_LIKES_USERS_FILE', $users), $names, ES::user($file->user_id)->getName());
