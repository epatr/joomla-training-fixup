...
class SocialEventAppAudiosHookNotificationLikes extends SocialAppHooks
{
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'audios.event.create' || $item->context_type == 'audios.event.featured') {

			// Get the audio object
			$audio = ES::audio($item->uid, SOCIAL_TYPE_EVENT, $item->context_ids);
			...
			if ($audio->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$item->title = JText::sprintf($this->getPlurality('APP_USER_AUDIO_NOTIFICATIONS_LIKES_YOUR_AUDIO', $users), $names);
			...
			if ($item->actor_id == $audio->user_id && count($users) == 1) {
				...
				$item->title = JText::sprintf($this->getGenderForLanguage('APP_USER_AUDIO_NOTIFICATIONS_LIKES_USERS_AUDIO', $actor->id), $actor->getName());
