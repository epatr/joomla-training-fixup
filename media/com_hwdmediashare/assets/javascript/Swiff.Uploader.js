	...
	reposition: function(coords) {
		...
		coords = coords || (this.target && this.target.offsetHeight)
			...
			: {top: window.getScrollTop(), left: 0, width: 40, height: 40}
                ...
                // HWD Mod dhorsfall 20120417
                // Get language direction
                var x = $('hwd-upload-browse');
                ...
                // HWD Mod dhorsfall 20120404
                // In Joomla admin, the prototype getOffsetParent() function gets incorrect 
                // values. Unsure why, but set them using Mootools instead.
                var position = $('hwd-upload-browse').getPosition();
                coords.left = position.x;
                coords.top = position.y;
                if (y == 'rtl')
                {
                	coords.right = 'auto';
                }
                
                // HWD Mod dhorsfall 20151118
                // Provide alterntive jQuery method to get vertical positioning. 
//                var position = jQuery('#hwd-upload-browse').position();
//                if (position.top > 0) {
